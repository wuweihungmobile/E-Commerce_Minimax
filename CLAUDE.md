...
### Rule 6 — （已移除）
- 2026-09-26 依使用者指示移除原「Token 預算」限制（使用者的訂閱本身已有用量上限）。
- 不再以 token 數量為由中斷、摘要重啟或揭露「超標」。
- 編號刻意保留：既有 Sprint 文件大量引用 Rule 編號（例如 Rule 3 出現在 51 份、Rule 12 出現在 90 份文件），重新編號會讓這些引用指到錯的規則。
