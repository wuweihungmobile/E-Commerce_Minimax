...
public interface ConversationRepository extends JpaRepository<Conversation, UUID> {
    ...
    @Query("SELECT c FROM Conversation c WHERE (c.initiatorId = :userId OR c.recipientId = :userId) AND c.isActive = true ORDER BY c.lastMessageAt DESC")
    Page<Conversation> findByUserIdOrderByLastMessageAtDesc(@Param("userId") UUID userId, Pageable pageable);

    @Query("SELECT c FROM Conversation c WHERE (c.initiatorId = :userId OR c.recipientId = :userId) AND c.isActive = true AND c.listingId = :listingId ORDER BY c.lastMessageAt DESC")
