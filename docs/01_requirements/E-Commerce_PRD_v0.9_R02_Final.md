# E-Commerce System — 規格文件 v0.9_R02_Final

> **狀態**: SDD 擴展版 · v0.8 全面整合 · 六項新需求 · B2B2C + CMS + ERP + 動態定價
> **建立日期**: 2026-03-23
> **升版日期**: 2026-03-31
...
> - v0.8 → v0.9：六項新需求整合——(1) 統一商品/服務模型含動態定價；(2) CMS 內容驅動銷售；(3) 一體化進銷存 (ERP)；(4) SaaS 平台化多租戶架構 (B2B2C)；(5) 友善前台 + 強大後台管理；(6) 網友開店 RBAC 權限矩陣。**本文件為完整獨立規格，已全面整合 v0.8 所有內容。**
> - v0.7 → v0.8：整合六份附屬文件為單一規格文件（`001_Master_Spec.md`、`005_M05_Spec.md`、`008_API_Errors.md`、`010_Supporting_RTM.md`、`011_Supporting_Errata.md`、`012_Supporting_Sprint-Entry.md`）
> - v0.6 → v0.7：Amanda 整體覆檢補正（G-8 M06 Phase 歧義消除；M05/M03/M01/M02 各有獨立 SPEC）
> - v0.5 → v0.6：Victoria PRD 審查修正
>   - §5 新增 3 張核心用戶人物誌（Buyer/Seller/Host）
>   - §9 新增 Phase 1 行為約束與成功指標
>   - §4 新增 MoSCoW 優先級校準表
...
### 6.9 M12 動態定價引擎（從 Phase 2 提前至 Phase 2-A）

**v0.9 變更：** M12 從 v0.8 的 Phase 2 提前至 Phase 2-A，因為用戶需求明確要求民宿支援動態價格。
...
### 9.6 M05 零售訂單履約（Phase 1 最小子集）（繼承 v0.8 §7.5）

| 方法 | 端點 | 說明 | Phase |
|------|------|------|-------|
| `POST` | `/api/orders` | 建立訂單（含冪等性 Key，**Phase 1 Payment Mock 直接推進狀態至 CREATED(=PAID)**） | **Phase 1** |
| `GET` | `/api/orders` | 買家/商家訂單列表（**Seller 可透過 `?merchantId={id}` 查詢店鋪訂單**；Buyer 查詢本人訂單） | **Phase 1** |
| `GET` | `/api/orders/:id` | 訂單詳情 | **Phase 1** |
| `PUT` | `/api/orders/:id/cancel` | 取消訂單（觸發狀態機，Phase 1 以 CREATED(=PAID) 為主要取消觸發點） | **Phase 1** |
| `GET` | `/api/orders/:id/state-log` | 查詢狀態機變更日誌 | **Phase 1** |

> **注意**：Phase 1 訂單僅支援單一類型（`RETAIL` 或 `BOOKING`），禁止混合訂單。支付環節為 Payment Mock，不串接真實金流。

### 9.7 M06 民宿預訂（繼承 v0.8 §7.6）

> **重要說明**：Phase 1 的 M06 **僅開放查詢與取消**，不含 POST 建立預訂。建立預訂（`POST /api/bookings`）屬 Phase 2 範圍。

| 方法 | 端點 | 說明 | Phase |
|------|------|------|-------|
| `GET` | `/api/v1/bookings` | 買家預訂列表 | **Phase 1** |
| `GET` | `/api/v1/bookings/:id` | 預訂詳情 | **Phase 1** |
| `PUT` | `/api/v1/bookings/:id/cancel` | 取消預訂（釋放日期格） | **Phase 1** |
| `GET` | `/api/v1/bookings/:id/state-log` | 查詢預訂狀態機日誌 | **Phase 1** |
| `POST` | `/api/v1/bookings` | 建立預訂（含日期格鎖定，冪等性 Key） | **Phase 2** |
| `PUT` | `/api/v1/bookings/:id` | 修改已確認的預訂（時間、地點等） | **Phase 2** |
| `POST` | `/api/v1/bookings/:id/remind` | 設定/發送預訂提醒 | **Phase 2** |
...
### 9.10 M17 租戶管理 API（v0.9 新增）

| 方法 | 端點 | 說明 | 角色 |
|------|------|------|------|
| `POST` | `/api/v2/tenants/apply` | 網友申請開店 | Buyer+ |
...
**[MoSCoW] 完整優先級表（Victoria 校準）**

**Must Have（必須有）— Phase 1 交付**

| 優先序 | 模組 | 功能 | 說明 |
|:----:|------|------|------|
| 1. | M03 | 會員系統核心 | 註冊/登入/JWT + Refresh Token + 黑名單 + RBAC |
| 2. | M01 | 商品中心核心 | 列表/詳情/分頁/篩選/搜尋 |
| 3. | M02 | 房源中心核心 | 列表/詳情/日期格查詢 + 地區/日期/人數篩選 |
| 4. | M05 | 訂單履約最小子集 | 訂單建立 + 狀態機流轉 + 取消 + 冪等性 |

**Should Have（應該有）— Phase 2 前期**

| 優先序 | 模組 | 功能 | 說明 |
|:----:|------|------|------|
| 5. | M04 | 購物車系統 | Redis Hash 存儲、優惠券、滿額折扣計算 |
| 6. | M06 | 預訂與日曆鎖定 | **Phase 1**：GET 查詢 + 取消；**Phase 2**：POST 建立預訂、Redis 分散式鎖防止 Overbooking、日期格索引 |
| 8. | M09 | 訊息通知中心 | Email/SMS/App Push 通知模板化 |
| 9. | M11 | 物流追蹤 | 第三方物流 API 介接、運費模板 |

**Could Have（可以有）— Phase 2 後期**

| 優先序 | 模組 | 功能 | 說明 |
|:----:|------|------|------|
| 10. | M08 | 評價與社交互動 | 圖文評論、星級評分、商家回覆 |
| 11. | M10 | 即時通訊（IM） | WebSocket 私訊、未讀計數、快捷回覆 |
| 12. | M12 | 動態定價引擎 | 平假日調價、早鳥優惠、長住折扣 |

**Won't Have（暫時不做）— Phase 2+ 明確排除**

| 優先序 | 模組 | 排除功能 | 說明 |
|:----:|------|---------|------|
| 13. | M01 | SKU 多規格管理、組合包、ElasticSearch 全文檢索 | Phase 2+ |
| 14. | M02 | PostGIS LBS 地圖模式、景點距離計算 | Phase 2+ |
| 15. | M03 | OAuth2 第三方登入、KYC 實名認證 | Phase 2+ |
| 16. | M05 | 電子發票開立、拆單、退款 | Phase 2+ |
| 17. | M07 | Saga Pattern 分散式交易 | Phase 2+ |
| 18. | M13 | 商家工作台（Seller/Host 儀表板） | Phase 2+ |
| 19. | M14 | 平台總管理台、Admin 全站配置 | Phase 2+ |
...
| TC-M05-006 | 建立 RETAIL 訂單 — 並發庫存扣減防護（v0.9_R02_Loop_01 新增） | SKU-001 的 `available_qty = 10`，100 個買家同時發出 `POST /api/v1/orders`（各 1 件） | 系統處理所有並發請求 | exactly 10 筆訂單成功（`status = CREATED`），`reserved_qty = 10`；其餘 90 筆回傳 `422 E-4003 INSUFFICIENT_STOCK_DUE_TO_CONCURRENT_ACCESS`；SKU-001 的 `available_qty` 最終為 0 |
| TC-M05-007 | 建立 RETAIL 訂單 — 庫存為 0 直接失敗 | SKU-002 的 `available_qty = 0` | 嘗試建立訂單 | 直接回傳 `422 E-4003`，不進入並發競爭 |
| TC-M05-006 | 建立訂單 — 無效 JWT | 未攜帶有效 JWT | POST /api/v1/orders | 401，E-2001 |
| TC-M05-007 | 建立訂單 — totalAmount 篡改 | totalAmount 與系統計算不符 | 系統校驗金額一致性 | 422，E-4006 |
...
_本文檔為 E-Commerce System 的完整合併規格文件 v0.9，所有內容具有同等約束力。v0.8 所有內容已完整整合，本文件可獨立使用，無需參照 v0.8。_

[Next Action] 請 Koala 審閱本規格文件 v0.9，確認後啟動 Phase 2-A 實作。
