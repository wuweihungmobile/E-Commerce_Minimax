...
**[Specification] 結算訂單歸屬與時區（Sprint 194／195，使用者拍板）**

先前「結算上一週已完成訂單」未定義以下單或完成時間歸週，實作以「下單時間落在該週、且結算當下已完成」歸屬，
導致**週間下單、下週才送達的訂單永遠不被任何一期結算**。明確規格如下：

| 項目 | 規格 |
|------|------|
| 結算週的時區 | **台灣時間（UTC+8）**：週一 00:00 ～ 週日 23:59:59.999999；排程於台灣週一 00:00 觸發 |
| 訂單歸屬 | 每筆訂單**恰好被結算一次**：訂單記錄它被哪張結算單結算（`orders.settled_statement_id`，NULL＝尚未結算）；每次結算納入「所有已完成（COMPLETED／DELIVERED）且尚未結算」的訂單，不論哪週下單；訂單在結算單產生**之後**才變成已完成，則由再下一次結算納入 |
| 認領方式 | 結算時以 `UPDATE ... WHERE settled_statement_id IS NULL` 原子標記；認領筆數不足（兩個結算同時搶同一批訂單）則回滾整張結算單 |
| 結算單 `REJECTED`（駁回，終態、資金未發生） | 釋放它認領的訂單與折入的調整單（`adjustment_statements` 還原為 `PENDING`），下一期重新結算 |
| 結算單 `FAILED`（撥款失敗，可重試） | **不釋放**訂單（重試撥款會把它改回 `APPROVED`，釋放會雙重撥款）；此時的退款比照 `APPROVED`／`PAID`，產生調整單於下一期折入 |
| 結算單 `REVERSED`（會計沖銷） | 不釋放（是否重新結算屬會計人工處理） |
| 退款調整的定位 | 以訂單所屬結算單（`settled_statement_id`）定位，不以下單日期猜測；尚未結算的訂單退款不動任何結算單（退款於該訂單被結算時由 `Payment.refundedAmount` 帶入） |
| 歷史訂單 | 不回填（NULL）：首次結算會一併納入所有歷史已完成訂單（週結算單過去在真實資料庫從未成功產生過） |
