# 【設計稿・已封存】SRD 資料庫 Schema 設計 v1.0

> **🔴 這份文件不是實作現況，不可作為開發依據。**
>
> **封存日期**: 2026-09-02（Sprint 111，DEF-062）
> **封存原因**: 本文件是 2026-04 設計期的產物，**從未與實作對齊過**。以「乾淨 DB 套完全部
> 70 個 Flyway 遷移」為權威基準逐表比對後確認：所記載的 16 張表**全部漂移**，
> 其中 117 個欄位在實作中不存在、55 個實作欄位未被記載，
> 且 `user_profiles`、`pricing_overrides` 兩張表**從未被實作**。
> 連 `product_inventory` 的主鍵都是錯的（文件寫 `id`，實作是 `sku_id`）。
>
> **實作現況請看**: [../SRD_Database_Schema.md](../SRD_Database_Schema.md)
>
> **為什麼保留這份**: 它記錄了「當初設計想的是什麼」——那是決策的歷史，直接刪掉等於
> 抹除「設計與實作為何分歧」這個資訊。需要考據設計意圖時看這份；需要知道資料庫長什麼樣
> 時看上面那份。
