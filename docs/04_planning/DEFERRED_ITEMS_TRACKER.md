...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-045 | `FREE_SHIPPING` 折扣型別在全系統無任何對應處理（選此型別的優惠券，買家拿不到任何優惠） | Sprint 100（優惠券斷鏈修復掃描時順帶發現） | `PromoService.computeDiscount` 對 `FREE_SHIPPING` 回傳 `BigDecimal.ZERO` 並註明「免運費由物流模組處理」，但全庫 grep 確認物流模組（`ShippingTemplateService`）**完全沒有任何 `FREE_SHIPPING` 相關邏輯**——main 程式碼僅 3 處提及此值：enum 定義、這個回 ZERO 的 case、一行 DTO 註解。店家若建立此型別的券，買家套用後折扣為 0 且運費照收。性質為「折扣型別未實作」，與 Sprint 100 修復的「斷鏈」不同類，未擴大範圍處理 | 需業務決策：免運券應如何作用（將 `shippingFee` 歸零？部分折抵？是否與 `ShippingTemplate` 的滿額免運疊加）；PRD 未定義此型別的計算規則 | 3 | ⚠️ 已記錄，待評估 |
