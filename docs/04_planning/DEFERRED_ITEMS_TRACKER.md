...
|----|------|-------------|---------|---------|---------|------|
| DEF-075 | **13 個權限碼在 `Permission` 枚舉不存在，5 個模組的端點對所有角色（含 SUPER_ADMIN）必定 403** | Sprint 128（修 DEF-073 時做全庫檢查發現） | Sprint 128 使用者拍板本輪只修 notification_template 一組，其餘登記待排程 | 需決定每個模組該授予哪些角色（5 個模組 × 角色判斷） | 8 | ⬜ 待排程。清單：`dashboard:read`、`faq:read/create/update/delete`、`knowledge:read/create/update/delete`、`media:read/create/update/delete`。根因同 DEF-073：生產唯一授權來源 `RolePermissionMapping.getAuthorities()` 只發枚舉內的碼，`SUPER_ADMIN` 的 `EnumSet.allOf` 也給不了枚舉沒有的東西。已由 `PreAuthorizePermissionCoverageTest` 的 `KNOWN_UNMAPPED_PENDING_DEF_075` 清冊守住（新增同類碼會讓測試失敗，且清冊不得包含已修好的碼）。詳見 [SPRINT_128_PLAN.md](SPRINT_128_PLAN.md) §6 |
| DEF-076 | ERP 建立採購單：後端品項 `@NotNull` 要 `listingId`，前端從不送出 → 建單必定 400 | Sprint 128（契約漂移掃描） | 使用者拍板本輪修三個主流程根因，ERP 一串留待下輪 | **需產品決策**：採購單品項要如何綁定平台 listing（前端表單目前沒有選擇器） | 5 | ⬜ 待排程。證據：`PurchaseOrderCreateRequest.java:39-40` vs `frontend/src/services/erp/purchaseOrder.ts:49-53`；呼叫點 `PurchaseOrderForm.tsx:205`。與 Sprint 127 DEF-071 同型（整條路徑從未運作） |
| DEF-077 | ERP 建立採購單：前端送 `unitPrice`，後端 `@NotNull` 要 `unitCost`（無 `@JsonProperty` 對映）→ 同樣必定 400 | Sprint 128（契約漂移掃描） | 同 DEF-076 | 無（單純欄位名對齊），但須與 DEF-076 一起修才會通 | 1 | ⬜ 待排程。`unitPrice` 為未知欄位被 Jackson 靜默丟棄（全庫無 `FAIL_ON_UNKNOWN_PROPERTIES`），`unitCost` 收到 null；另 `PurchaseOrderService.java:98` 對 null 呼叫 `.multiply()` 會 NPE |
| DEF-078 | ERP 採購單檢視／編輯頁必定顯示「載入採購訂單失敗」：回應的 `expectedDeliveryDate` 恆為 null，前端對 null 呼叫 `.split()` | Sprint 128（契約漂移掃描） | 同 DEF-076 | 需決定 `expectedDeliveryDate` 是補進實體/mapper，還是前端改為容忍 null | 3 | ⬜ 待排程。`PurchaseOrderDto.java:36` 有欄位但 mapper 從不填、實體也無此欄位；`PurchaseOrderForm.tsx:103` 的 TypeError 被同檔 :116 catch 吞掉轉成 setError，整頁不可用 |
| DEF-079 | ERP 供應商「檢視／編輯」兩頁必定失敗：`GET /v2/dashboard/suppliers/{id}` 後端只有 PUT 沒有 GET mapping | Sprint 128（契約漂移掃描） | 同 DEF-076 | 無（補一支 GET 端點） | 2 | ⬜ 待排程。`ErpController.java` supplier 區塊只有三個 mapping，無單筆 GET；因 `GlobalExceptionHandler.java:117` 的 catch-all 優先序高於 `DefaultHandlerExceptionResolver`，實際回應是 500 + `E-9900` 而非 405。呼叫點 `SupplierForm.tsx:50` |
...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-081 | ERP 採購單列表卡片標題永遠空白：前端讀 `orderNumber`，後端只產生 `poNumber` | Sprint 128（契約漂移掃描） | 使用者拍板本輪只修三個主流程根因 | 無 | 1 | ⬜ 待排程。`PurchaseOrderDto.java:24` vs `purchaseOrder.ts:29`，渲染點 `purchase-orders/page.tsx:139` |
| DEF-082 | ERP 採購單列表「供應商」永遠空白：`supplierName` 後端 mapper 從不填（實體亦無此欄位） | Sprint 128（契約漂移掃描） | 同上 | 需決定是 mapper 補查供應商名，還是前端改用 supplierId 另查 | 2 | ⬜ 待排程。`PurchaseOrderDto.java:28` 有欄位但無人賦值 |
| DEF-086 | M09 通知模板管理實際只有唯讀列表：前端完全沒有建立與編輯模板的 UI | Sprint 128（修 DEF-073 時發現） | DEF-073 已修好後端權限（端點現在可用），但前端缺頁面屬功能缺口非缺陷修復 | 需 UI/UX 設計 | 5 | ⬜ 待排程。`notification.ts:90/95` 有 createTemplate/updateTemplate 函式，但無任何元件呼叫 |
| DEF-087 | 房源搜尋的 `keyword` 參數後端完全不使用，搜尋永遠回傳未過濾的全部房源 | Sprint 128（契約漂移掃描） | 同上 | 無 | 3 | ⬜ 待排程。前端 `room.ts:100` 送出 keyword、`RoomList.tsx:40` 為呼叫端；後端 `RoomService.java:62-64` 的 keyword 分支主體未真正使用該值，並連帶使 location 過濾失效 |
| DEF-092 | 5 個權限碼僅 SUPER_ADMIN 到得了：`cms:read/create/update/publish`、`notification:create` | Sprint 128（修 DEF-073 時做全庫檢查發現） | 與 DEF-075 性質不同——註解帶 `hasRole('SUPER_ADMIN') or ...` fallback，**不是必定 403** | **需產品判斷**：CMS/發送通知是刻意的平台專屬功能，還是漏授權給店主 | 3 | ⬜ 待排程。已由 `PreAuthorizePermissionCoverageTest.superAdminFallbackOnlyCodesAreUnchanged` 自動辨識並守住，新增同類碼會失敗。詳見 [SPRINT_128_PLAN.md](SPRINT_128_PLAN.md) §6 |
...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-080 | 知識庫建立文章必定 400：後端 `@NotNull` 要 `authorId`、`@NotBlank` 要 `slug`，前端 `CreateArticleRequest` 兩者皆無 | Sprint 128（契約漂移掃描） | 驗證者由 high 下修為 low：**前端零呼叫者**（`createKnowledgeArticle` 未被任何元件匯入），屬死路徑 | 需先決定知識庫建立功能是否要接上前端（接上才有修的意義） | 3 | ⬜ 待排程。`CreateKnowledgeArticleRequest.java:23-24/30-32` vs `knowledge.ts:43-52`。另 `authorId` 值須為存在的 User UUID（`KnowledgeBaseService.java:193`） |
| DEF-083 | 知識庫文章標籤 `tags` 全鏈路未接線：後端 mapper 從不填 → 列表頁標籤區塊永遠不渲染 | Sprint 128（契約漂移掃描） | 同上 | 無 | 2 | ⬜ 待排程。`KnowledgeArticleDto.java:32` 有欄位、唯一 mapper 從不填；渲染點 `dashboard/knowledge/page.tsx:181` |
| DEF-084 | 合併結帳含空白的電話必定 400：前端驗證時剝除空白但送出時不剝除 | Sprint 128（契約漂移掃描） | 使用者拍板本輪只修三個主流程根因 | 無 | 1 | ⬜ 待排程。`checkout/mixed/page.tsx:122` 驗證用 `.replace(/\s/g,'')` 但 :147 送出原值；後端 `CheckoutDto.java:62` 的 `@Pattern(^[0-9]{8,15}$)` 不容空白 |
| DEF-085 | 更新媒體只改 tags 必定 400：後端 `UpdateMediaRequest.categoryId` 為 `@NotNull`，前端宣告為選填 | Sprint 128（契約漂移掃描） | 同上 | 需決定 categoryId 該放寬為選填，還是前端強制帶上原分類 | 1 | ⬜ 待排程。`UpdateMediaRequest.java:18-19` vs `media.ts:43-46` |
| DEF-088 | 冪等衝突回應硬編碼 `"E_6005"`（底線），與 `ErrorCode` 正式 wire code `"E-6005"`（連字號）及前端對照表不符 | Sprint 128（契約漂移掃描） | 同上 | 無 | 1 | ⬜ 待排程。兩處：`BookingController.java:106`、`CheckoutController.java:68`。前端對照表 `booking.ts:116`／`checkout.ts:55` 皆為連字號，故 409 時顯示通用文案而非專屬提示 |
| DEF-089 | `cms.ts` 的 `uploadMedia` 以 multipart FormData 打 `/v2/media/upload`，但該端點只吃 JSON `@RequestBody` 且有 5 個必填欄位 | Sprint 128（契約漂移掃描） | 同上 | 需決定改打 `/v2/dashboard/media/upload-multipart`（既有 multipart 端點）或改送 JSON | 2 | ⬜ 待排程。`cms.ts:231-232` vs `MediaController.java:70-72`；回應亦無 `mediaId` 欄位 |
| DEF-090 | 更新採購單時 `expectedDeliveryDate`／`items` 被靜默丟棄：後端 `UpdateRequest` 只有 `notes` | Sprint 128（契約漂移掃描） | 同上 | 無（後端註解已寫明 items 刻意不允許更新，僅前端型別誤導） | 1 | ⬜ 待排程。`PurchaseOrderUpdateRequest.java:15-17` vs `purchaseOrder.ts:56-64` |
| DEF-091 | `listing.ts` 的 `PriceBreakdown` 宣告 `price`，後端 `PricingDto.PriceBreakdown` 只產生 `basePrice`/`adjustedPrice` | Sprint 128（契約漂移掃描） | 同上 | 無 | 1 | ⬜ 待排程。`PricingDto.java:132-138` vs `listing.ts:21-23` |
...
**文件版本**: v2.28
**最後更新**: 2026-09-04（Sprint 127：`ErrorCode` 孤兒碼掃描找到 `DEF-070`（採購單收貨超收無驗證）並修復；過程中檢視前端呼叫端另發現 `DEF-071`（ERP 收貨確認頁三缺陷疊加導致完全無法使用），經 AskUserQuestion 拍板併入本輪一起修。兩項皆為本輪發現同輪修復，移入已完成延後項目。詳見 [SPRINT_127_PLAN.md](SPRINT_127_PLAN.md)）
