...
|----|------|-------------|---------|---------|---------|------|
| DEF-050 | `ProductInventoryService.reserveForOrder` 與 DEF-046 完全同型的讀後寫競態——併發下單可**超賣實體庫存** | Sprint 102（修復 DEF-046 後依 Action Item 做同類競態橫向掃描時確認） | 本輪範圍已明確界定為 DEF-046（優惠券額度），依 Rule 3（精準改動）不順手擴大到庫存模組；該模組需要自己的併發測試與紅燈驗證，併入本 Sprint 會使範圍失控。**但缺陷本身已確認存在，不是推測**：`reserveForOrder` 先呼叫 `inventory.hasAvailableStock(qty)`（讀 `totalQty - reservedQty`），再 `inventory.reserve(qty)` + `productInventoryRepository.save(inventory)`（整個實體寫回）——與 DEF-046 修復前的結構完全相同：兩張並行訂單搶同一個 SKU 的最後一件，會同時通過檢查而雙雙預扣，且兩次 `save()` 互相覆蓋使 `reservedQty` 少算。`releaseForOrder`/`deductForOrder` 同樣是「讀出 → 改欄位 → save」。**嚴重性高於 DEF-046**：DEF-046 超發的是折扣額度（可事後補償），本項超賣的是**實體商品**（賣出去的貨不存在，需退款+客訴處理）。Sprint 88（AI-2422）建立這套三段式庫存機制時未考慮併發 | 無前置需求——DEF-046 的修法可直接複用：條件式 UPDATE（`UPDATE product_inventory SET reserved_qty = reserved_qty + :qty WHERE sku_id = :id AND total_qty - reserved_qty >= :qty`），回傳 0 筆即庫存不足。測試同樣需真實 DB 多執行緒（可複用 `M11PromoConcurrencyIntegrationTest` 的 raceFor 結構）| 3 | ⚠️ 已記錄，**建議排入 Sprint 103** |

> **DEF-023**（Booking 付款擁有權檢查缺口，IDOR）已於 Sprint 68 修復並結案；**DEF-024**（`OrderService.updateOrderStatus` 跨租戶 IDOR）已於 Sprint 70 修復並結案；**DEF-038**（`TenantContextFilter` ADMIN 跨租戶 `X-Tenant-ID` header 無驗證信任）已於多 Sprint 測試強化計劃後的獨立追蹤任務修復並結案；**DEF-040**（`SettlementController` 結算單審核租戶過濾缺口）已於 Sprint 81 修復並結案；**DEF-034**（`CmsService` 公開瀏覽端點租戶範圍設計）已於 Sprint 82 修復並結案；**DEF-041**（`RoomService`/`ProductService` 寫入層租戶擁有權缺口，含根因修復）已於 Sprint 84 修復並結案；**AI-2419**（PRD §6.7.2 M16 採購審批金額上限機制，Sprint 10 遺漏）已於 Sprint 85 修復並結案；**AI-2420**（PRD §6.2.1 M07 跨週期退款調整單機制 + CREDIT_NOTE 雙重授權）與**DEF-042**（`admin:read`/`admin:write` 權限缺口導致結算審核流程 403 不可達）已於 Sprint 86 修復並結案；**AI-2421**（PRD §14.3.1 Phase 2-B 收貨地址簿）已於 Sprint 87 完成並結案；**AI-2422**（PRODUCT 商品訂單結帳流程前端串接 + 混合購物車修復 + 庫存預扣/扣帳/釋放）已於 Sprint 88 完成並結案；**AI-2423**（PRD §6.7.2 M16 採購審批門檻前端串接）已於 Sprint 89 完成並結案；**AI-2424**（PRD §6.2.1 M07 結算逆轉發起/確認表單）已於 Sprint 90 完成並結案；**AI-2425**（PRD §6.10 M18 客服工單子系統，後端全部 + 買家前端）已於 Sprint 91 完成並結案；**AI-2426**（M18 客服工單子系統，店家/平台前端）已於 Sprint 92 完成並結案，詳見下方「已完成延後項目」。
...
| DEF-037 | `ShippingTemplateService.calculateFee` 未做租戶過濾，可查詢他租戶運費模板設定 | Sprint 76（探查 `LogisticsService`/`ShippingTemplateService` 範圍時發現） | 探查確認 `calculateFee(templateId, orderAmount)`（`/v2/shipping-templates/{id}/calculate-fee`，僅要求 `product:read`）未驗證 `templateId` 是否屬於當前租戶，任一登入使用者可代入他租戶的 `templateId` 取得其 `feeType`/`fixedAmount`/`freeThreshold` 運費設定；`createTemplate`/`getTemplates`/`updateTemplate`/`deleteTemplate` 皆已由 Controller 傳入 `TenantContext` 取得的 `tenantId` 並在 Service 內用 `.filter(t -> tenantId.equals(t.getTenantId()))` 正確隔離，僅 `calculateFee` 遺漏。非典型 IDOR（無寫入/竄改風險，回傳內容僅運費計算參數，不含 PII），但與買家結帳流程的既有慣例（`calculateFeeForTenant(tenantId, ...)` 已依賴呼叫端傳入信任的 `tenantId`）是否刻意設計為「僅需模板 ID 即可跨租戶試算運費」以支援買家瀏覽/比價情境，或應收斂為僅限模板所屬租戶存取，需業務判斷，本 Sprint 依範圍僅記錄不變更程式碼 | 需業務決策：`calculateFee` 是否應比照 `updateTemplate`/`deleteTemplate` 加上租戶過濾（若加上，需確認是否影響買家跨租戶比價/結帳試算的既有使用情境） | 1 | ⚠️ 已記錄，**不排入排程**（待使用者決策） |
| DEF-043 | ROOM 訂房結帳流程（`checkout/page.tsx`）建立預訂成功後呼叫 `DELETE /v2/cart` 清空整個購物車，若購物車混有尚未結帳的 PRODUCT 項目會被誤刪 | Sprint 88（補齊 PRODUCT 結帳前端串接、修復 PRODUCT 側同類缺陷時發現 ROOM 側也有對稱問題） | 探查發現購物車為 ROOM/PRODUCT 共用（`RedisCartService`），`OrderService.createOrderFromCart`（PRODUCT 分支）先前也有相同缺陷已於本 Sprint 修復（僅移除已處理項目，見 AI-2422）；但 ROOM 側的結帳成功後由**前端**直接呼叫 `DELETE /v2/cart` 整個清空，屬於另一條已上線、範圍與風險需獨立評估的既有流程（`/v2/bookings` 而非 `/v2/orders`），故不併入本 Sprint 修復 | 需評估修法：改為僅移除已處理的 ROOM 項目（比照本 Sprint PRODUCT 側改用 `removeItem` 而非 `clearCart` 的修法），或改由後端 `createRoomOrder` 統一處理購物車移除 | 1 | ⚠️ 已記錄，**不排入排程** |
| DEF-044 | 已付款訂單（`PAID`/`REFUNDED`）退款/取消時無庫存自動回補（restock-on-refund） | Sprint 88（AI-2422 訂單庫存串接時，PO 裁示範圍界定為「僅處理預扣→扣帳單向流程」，退款回補列為範圍外） | 本 Sprint 新增的 reserve-at-creation/deduct-at-payment/release-on-cancel-before-payment 三段式庫存機制，僅覆蓋「取消前尚未付款」的釋放情境；已付款訂單走退款流程（`PaymentStateService.refundOrderPayment`）時，先前已透過 `deductStock` 正式扣除的 `totalQty` 目前**不會**因退款而回補，與 M07 結算/退款邏輯有交互，需獨立設計（例如是否應恢復可售庫存、是否與 `SettlementAdjustment` 共用同一觸發點） | 需業務決策：退款是否應自動回補庫存（可售）；若是，回補時點（退款成立時 vs 商品實際retour確認時）需與供應鏈/退貨流程一併考慮 | 2 | ⚠️ 已記錄，**不排入排程** |
...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-047 | ROOM（訂房）訂單無法套用優惠券（PRD US-010 未落地） | Sprint 100（使用者明確裁定本輪只修 PRODUCT 路徑） | PRD US-010 明文「作為預訂買家，我希望在結帳時套用優惠碼，以便節省費用」，但 `OrderService.createRoomOrder` 不走購物車（直接以 `listingId` + 日期建單），完全沒有促銷碼路徑。Sprint 100 修復的是 PRODUCT 結帳斷鏈；ROOM 要支援需新增 API 契約欄位與訂房結帳前端輸入框，規模明顯超出「修復斷鏈」，使用者已於動工前透過 AskUserQuestion 裁定延後 | 需設計 `POST /v2/orders`（ROOM 分支）的促銷碼參數契約，並改版訂房結帳前端；可複用 Sprint 100 已建立的 `resolveValidPromoForCheckout`/`commitPromoUsage`/`refundPromoUsage` | 5 | ⚠️ 已記錄，待排程 |
| DEF-048 | 混合購物車（PRODUCT + ROOM）的優惠券折扣基數，購物車顯示與結帳實收不一致 | Sprint 101（修復 DEF-045 時識別出的相鄰既有行為） | `RedisCartService.getCartWithPromo`／`applyPromoCode` 以 `cart.getTotalAmount()`（**含 ROOM 項目**）作為 PERCENTAGE/FIXED_AMOUNT 的折扣基數，而 `OrderService.createOrderFromCart` 只以 **PRODUCT 項目小計**為基數。同時放了 PRODUCT 與 ROOM 的購物車，顯示折扣會大於實際折扣。此行為早於 Sprint 100 即存在，與 `FREE_SHIPPING` 正交，Sprint 101 依 Rule 3（精準改動）刻意未順手修改 | 需產品確認：優惠券是否本就該折抵 ROOM 項目金額（若是，該修的是結帳端而非顯示端） | 2 | ⚠️ 已記錄，待評估 |
| DEF-049 | 整合測試套件執行時間持續成長，已撞穿雲端 CI 的 job 逾時預算 | Sprint 101（雲端 CI run 33466882327 的 `Backend Integration Tests & Package` job 於 25m19s 被 cancel 時發現） | 根因**不是任何測試失敗**——該 job 步驟 1-8（checkout/JDK/服務連線/DB reset/compile）全綠，只有第 9 步 `Run Integration Tests` 撞到 `timeout-minutes: 25` 被砍。實測數據：本地 30 個整合測試類別、392 個測試累計 861 秒（14.4 分），**平均每類 28.7 秒且分布極平坦**（最慢 37.9s、第 10 名 31.9s）——這是 **Spring context 啟動時間主導**的結構特徵，不是單一測試異常，因此無法靠優化個別測試改善。雲端 ubuntu-latest 約比本地（Apple Silicon）慢一倍，加 setup/compile 實測需 ~34 分。Sprint 101 已將該 job 預算調為 45 分**作為止血**，並於 run 33471691304 實測驗證：該 job 需 **29m48s**（success），餘裕 34%。🔴 **此警報已於 Sprint 102 更正**：S102 多加了 5 個整合測試，該 job 反而只跑 **23.1 分**（run 33489182490，success，餘裕 48.7%）。三個樣本為 25m19s（撞 25 分預算被 cancel）／29m48s／23.1m——**雲端 runner 的 run-to-run 變異（落差 29%）遠大於單一 Sprint 新增測試造成的成長**，原記載「距離下一次撞穿只剩約 15 分鐘」是把一次取樣當成趨勢，急迫性沒有那麼高。長期結構性成長仍然成立（見下方 (1) 的更正）。但每個 Sprint 新增整合測試都會逼近上限，**單純繼續調高數字不是解法**。同一結構也影響 `Backend Unit Tests`（`timeout-minutes: 12`，本次實測 7m00s、已用 58%），目前仍有餘裕故未動，但成長來源相同 | 🔴 **方向 (1) 已於 Sprint 102 查證推翻，勿再依它動工**：`pom.xml` 的 failsafe（與 surefire 相同）設定為 `forkCount=1` + `reuseForks=false`（原註解寫明「Ensure each test class runs in a separate JVM to avoid SecurityContext pollution」），**每個測試類別跑在各自的 JVM，Spring context 快取本來就無法跨類別共用**——收斂 `@MockBean`/`@ActiveProfiles` 設定不會有任何效果。本項原記錄的「30 個類別平均 28.7 秒且分布極平坦」不是「context 設定分歧」造成，而是 `reuseForks=false` 的必然結果：每一類都付一次完整的 context 啟動，所以分布當然平坦。**真正的第一支槓桿是 `reuseForks`**——需先確認當初「SecurityContext 汙染」的顧慮是否仍成立（可能已被後來的 `TenantContext.clear()`／`SecurityContextHolder.clearContext()` 慣例覆蓋），或改用 `@DirtiesContext` 對少數真正會汙染的類別精準處理，取代全域關閉 fork 重用。剩餘方向：(2) `forkCount` + `parallel` 平行化（需先確認測試間無共用 DB 狀態衝突，本專案整合測試皆打同一個 postgres，風險需評估）；(3) 雲端 job 分片（依模組拆成多個 job 並行）。**動工順序已由 Sprint 102 的查證改寫**：先評估 (1') `reuseForks=true` 的可行性（單一設定變更，影響最大），再考慮 (2)(3) | 5 | ⚠️ 已記錄，待評估 |
...
**文件版本**: v2.25
**最後更新**: 2026-07-06（Sprint 77：探查 `NotificationService`/`NotificationTemplateService` 範圍時，追蹤 `TenantContext` 信任來源發現 `TenantContextFilter` 對 `ADMIN` 角色的 `X-Tenant-ID` header 無驗證信任（架構審查項目），新增 `DEF-038`（🔴 高優先級，待業務/架構決策 `ADMIN` 角色租戶歸屬語意，本 Sprint 僅記錄未修改程式碼，不阻塞 Sprint 77 本身的通知模組測試工作）
