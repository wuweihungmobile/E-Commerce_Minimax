...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-191 | `SELLER` 角色缺 `order:create`，無法呼叫 `POST /v2/logistics` 建立物流單完成出貨 | Sprint 151（`DEF-188` 手動 Docker 驗證時發現） | `RolePermissionMapping`：`SELLER` 只有 `ORDER_READ`/`ORDER_UPDATE`，沒有 `ORDER_CREATE`；`STORE_OWNER` 有 `ORDER_CREATE`，`STORE_STAFF` 兩者皆無。`LogisticsController.createLogistics` 要求 `order:create`，導致 `SELLER` 可以把訂單從 `PAID` 確認到 `CONFIRMED`（有 `order:update`），卻無法建立物流單完成下一步出貨——相鄰兩步驟所需最低角色不一致，`RolePermissionMapping.java` 其餘角色限制皆有明確理由註解，唯獨此處沒有，較像既有疏漏而非設計。以 `STORE_OWNER` 角色重跑整段流程驗證程式邏輯本身正確、缺口純屬既有權限矩陣，非本輪程式碼迴歸。是否放行 `SELLER` 或改 `createLogistics` 要求 `order:update`（語意上更貼近「管理訂單履約」）屬業務決策，未擅自變更。詳見 [SPRINT_151_PLAN.md](SPRINT_151_PLAN.md) §5.3 | 需業務面拍板權限矩陣調整方向 | 1 | ⚠️ 已記錄，待排程 |
