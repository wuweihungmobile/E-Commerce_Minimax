...
| DEF-057 | `KnowledgeBaseService.incrementViewCount` 是本類別唯一沒有租戶範圍的方法，A 租戶使用者可遞增 B 租戶文章的瀏覽數 | Sprint 106（修復 DEF-055 改寫該方法時發現） | 本類別其餘方法一律用 `findByIdAndTenantId`，姊妹服務 `FaqService.incrementViewCount` 也有租戶範圍，只有這裡是無範圍的 `findById`；端點為 `@PreAuthorize("hasAuthority('knowledge:read')")`，故任一持有該權限的他租戶使用者都能遞增。**後果是排名操縱而非資料外洩**——`KnowledgeArticleRepository.findPopularByCategoryId` 以 `viewCount DESC` 取熱門文章。Sprint 106 只改併發語意、不改權限語意：加上 `AND tenant_id = ?` 會讓原本會成功的跨租戶呼叫改為 404，屬行為變更且是安全議題而非該 Sprint 主題，依 Rule 3 未順手擴大。已在 repository 與 service 的 javadoc 各留一則說明，避免下一輪掃描把「沒有租戶條件」誤讀為疏漏而順手補上 | 需確認是否有「跨租戶瀏覽知識庫文章」的既有使用情境（比照 DEF-037 對 `calculateFee` 的同型疑問）；修法本身是一行 WHERE 條件，但需要租戶隔離的紅燈驗證 | 1 | ⚠️ 已記錄，待處理 |
| DEF-056 | `ChatService` 未讀計數為讀後寫，併發訊息會少計 | Sprint 105（同上掃描） | `conversation.setRecipientUnreadCount(conversation.getRecipientUnreadCount() + 1)`（`ChatService:160,162`）後接 `save()`，同一對話的併發訊息會互相覆蓋，收訊方的未讀徽章偏低。**自癒型缺陷**：使用者開啟對話後計數歸零，錯誤不累積。修法比前述幾處麻煩一些——同一次 `save()` 還寫入 `lastMessageId`／`lastMessagePreview`／`lastMessageAt`，那三個欄位「後寫入者覆蓋」在語意上是**正確**的（最後一則訊息本來就該是最後那則），只有計數欄位需要原子化，故不能整段照搬原子 UPDATE，需拆成「計數相對遞增 + 其餘欄位絕對賦值」的單一敘述 | 無前置需求 | 2 | ⚠️ 已記錄，低優先 |
| DEF-052 | `-Pintegration-test` 是一個**不存在的 Maven profile** | Sprint 104（查證 DEF-049 前提時發現） | failsafe 其實是無條件綁在 `verify` 階段上的，`-P` 旗標對它毫無作用（Maven 只會對不存在的 profile 發出警告，不報錯）。（Sprint 104 後 `pom.xml` 確實有了一個 `<profiles>` 區段，但那是 `skip-unit-tests`、且由 property 啟用而非 `-P`；`integration-test` 這個 profile 仍然不存在。）`Makefile:102` 的說明文字與 `pom.xml` 的 failsafe 註解都把它與 Spring 的 `@ActiveProfiles("integration-test")` 混為一談，兩者毫無關係。**無功能影響**，但會誤導——DEF-049 自己的記錄就引用了這個不存在的 profile。依 Rule 3，修正它不是 Sprint 104 的必要工作，未順手改動 | 無前置需求（改 3 處文字：Makefile 說明、pom 註解、以及日後任何引用） | 1 | ⚠️ 已記錄，待處理 |
