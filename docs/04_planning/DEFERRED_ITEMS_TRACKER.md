...
| DEF-049 | 整合測試套件執行時間持續成長，已撞穿雲端 CI 的 job 逾時預算 | Sprint 101（雲端 CI run 33466882327 的 `Backend Integration Tests & Package` job 於 25m19s 被 cancel 時發現） | 根因**不是任何測試失敗**——該 job 步驟 1-8（checkout/JDK/服務連線/DB reset/compile）全綠，只有第 9 步 `Run Integration Tests` 撞到 `timeout-minutes: 25` 被砍。實測數據：本地 30 個整合測試類別、392 個測試累計 861 秒（14.4 分），**平均每類 28.7 秒且分布極平坦**（最慢 37.9s、第 10 名 31.9s）——這是 **Spring context 啟動時間主導**的結構特徵，不是單一測試異常，因此無法靠優化個別測試改善。雲端 ubuntu-latest 約比本地（Apple Silicon）慢一倍，加 setup/compile 實測需 ~34 分。Sprint 101 已將該 job 預算調為 45 分**作為止血**，並於 run 33471691304 實測驗證：該 job 需 **29m48s**（success），餘裕 34%——**距離下一次撞穿只剩約 15 分鐘的成長空間**。但每個 Sprint 新增整合測試都會逼近上限，**單純繼續調高數字不是解法**。同一結構也影響 `Backend Unit Tests`（`timeout-minutes: 12`，本次實測 7m00s、已用 58%），目前仍有餘裕故未動，但成長來源相同 | 需評估三個方向：(1) `@SpringBootTest` context 快取命中率——確認各測試類別的 context 設定是否因 `@MockBean`/`@ActiveProfiles` 差異而無法共用，若能收斂成少數幾種 context 可大幅縮短；(2) surefire/failsafe `forkCount` + `parallel` 平行化（需先確認測試間無共用 DB 狀態衝突，本專案整合測試皆打同一個 postgres，風險需評估）；(3) 雲端 job 分片（依模組拆成多個 job 並行）。動工前應先量測 context 重建次數，確認 (1) 的改善空間再決定是否需要 (2)(3) | 5 | ⚠️ 已記錄，待評估 |
