...
|----|------|-------------|---------|---------|---------|------|
| DEF-040 | `SettlementController` 的 `/v2/admin/settlements/pending`/`approve`/`reject` 完全不做租戶過濾，任一租戶的 ADMIN 可審核/批准/駁回他租戶結算單 | Sprint 80（開發 AI-2416 Transfer 分潤時發現） | 開發 Transfer 分潤功能時追蹤 `SettlementReviewer` 的呼叫路徑，發現 `approveStatement`/`rejectStatement`/`getPendingReviewStatements` 三個方法（`SettlementReviewer.java`）皆直接 `findById`/`findByStatusOrderByGeneratedAtDesc` 查詢，完全沒有依 `TenantContext` 做租戶過濾；對應 `SettlementController` 的 `/v2/admin/settlements/**` 三個端點僅檢查 `admin:read`/`admin:write` 權限（任一租戶的 `ADMIN` 角色皆具備），未限制只能操作自己租戶的結算單。**風險因 Sprint 80 新增的 AI-2416 Transfer 分潤功能而放大**：`approveStatement` 現在會觸發真實（test mode）Stripe transfer，若此為漏洞而非刻意設計，代表任一租戶的 ADMIN 理論上可核准其他租戶的結算單並觸發其資金轉移。與 DEF-038（`TenantContextFilter` ADMIN 跨租戶）同屬「新功能忘記加租戶擁有權檢查」模式，但發生在不同層（Service 內部查詢邏輯，非 filter 層）。是否為刻意設計（例如「平台級財務審核本就該由跨租戶的營運團隊處理」）尚未確認，本 Sprint 依範圍僅記錄不變更程式碼 | 需業務/架構決策：(1) Settlement 審核流程的角色定位應為「租戶內管理員審核自己租戶」還是「平台級財務團隊跨租戶審核」；(2) 若為前者，`approveStatement`/`rejectStatement`/`getPendingReviewStatements` 需補上 `TenantContext` 過濾（比照 `SettlementStatementRepository.findByIdAndTenantId` 既有模式）；(3) 若為後者，應改為要求 `SUPER_ADMIN` 角色而非 `admin:write` 權限（比照 `AdminController` 慣例），避免任一租戶的 `ADMIN` 意外持有此能力 | 3 | ⚠️ 已記錄，**不排入排程**（待使用者決策，Sprint 80 發現但非本 Sprint 範圍） |

> **DEF-023**（Booking 付款擁有權檢查缺口，IDOR）已於 Sprint 68 修復並結案；**DEF-024**（`OrderService.updateOrderStatus` 跨租戶 IDOR）已於 Sprint 70 修復並結案；**DEF-038**（`TenantContextFilter` ADMIN 跨租戶 `X-Tenant-ID` header 無驗證信任）已於多 Sprint 測試強化計劃後的獨立追蹤任務修復並結案，詳見下方「已完成延後項目」。
