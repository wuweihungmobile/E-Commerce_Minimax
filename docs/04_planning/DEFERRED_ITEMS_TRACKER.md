...
| DEF-195 | `TenantApplicationRequest.businessLicenseUrl`（開店申請營業執照網址）無協定白名單驗證，且全代碼庫零讀回/消費端 | Sprint 155（查證 DEF-193/194 時，比對 Tenant 相關 DTO 的其他 URL 欄位順帶發現，未在本輪原定範圍內） | `TenantService`（第 114 行）確實把 `request.getBusinessLicenseUrl()` 寫入並持久化至 `TenantApplication.businessLicenseUrl`，但 `grep` 全庫確認此欄位**沒有任何 Response DTO 曾經序列化它**（不同於 DEF-104/DEF-194，連公開讀回端點都不存在），前端也零消費點。比 DEF-104/194 更難構成攻擊鏈（連 sink 存在的前提——讀回——都不成立），但寫入路徑本身缺協定驗證仍是輸入驗證品質缺口，如實記錄、不擅自擴大本輪修復範圍 | 無（若未來新增審核台顯示營業執照連結，需先補協定驗證再串接） | 1 | ⚠️ 已記錄，**不排入排程**（本輪範圍外新發現，如實記錄） |
