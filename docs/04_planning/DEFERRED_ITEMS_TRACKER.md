...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-114 | AdminService.approveTenantApplication：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 實測後果非統計數字類、也非自癒型，而是會留下永久性的髒資料/孤兒資源：(1) 產生兩個獨立、都是 status=ACTIVE 的 Tenant（AdminService.java:828-836 每次呼叫都 new Tenant().builder()...save()，完全不檢查該 application 是否已被處理過）；(2) 產生兩筆 tenant_members STORE_OWNER 記錄，分屬不同 tenantId（同檔 ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-115 | AdminService.rejectTenantApplication：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 跨兩個 API 交易的遺失更新（lost update），不是統計數字誤差，而是「業務狀態與實際系統狀態互相矛盾」的資料完整性/存取控制問題：TenantApplication 最終落地的 status 可能是 REJECTED（且帶有 rejectionReason），但同一筆申請對應的 Tenant 已被建成 status=ACTIVE、tenant_members 已寫入該使用者為 STORE_OWNER、且 User.role ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| DEF-117 | AdminService.updateTenantStatus：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若兩個 SUPER_ADMIN 併發呼叫 PUT /admin/tenants/{tenantId}/status 對同一租戶下達「意圖相反」的狀態轉換（例如租戶目前為 SUSPENDED，一人要求 ACTIVE 復活、另一人要求 TERMINATED 終止），會發生典型 lost update：Tenant 實體無 @Version 樂觀鎖（已確認 Tenant.java 完全沒有 @Version 欄位），tenant.setSta... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| DEF-138 | SettlementReversalService.initiateReversal：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 不是金額競態（CreditNote.amount 在 confirmReversal 裡固定用 statement.getNetSettlementAmount().negate()，不受 initiateReversal 這個競態影響），也沒有真正打破「雙重授權」這個安全邊界本身（無論最後 reversal_initiated_by_role 被覆寫成 SUPER_ADMIN 或 CFO，confirmReversal 第 97 行的... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| DEF-140 | TenantService.acceptInvite：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 屬 security_or_access 類，但影響面比初步描述窄：(1) tenant_members.status 確實會發生 lost update——TenantService.java 的 acceptInvite (L585-591)、declineInvite (L614-619)、removeMember (L739-755) 都是「findByTenantIdAndUserId 讀→過濾狀態→setStatus→sav... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-141 | TenantService.createApplication：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 非金流/非安全性後果，屬於「重複業務記錄」類。若兩個並行請求都通過 existsByUserIdAndStatusIn 檢查，會各自 INSERT 一筆同一使用者的 PENDING TenantApplication（TenantService.java:88-108）。這不是死資料：AdminService.getPendingTenantApplications()（line 797-807）用 findByStatus(PENDI... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-142 | TenantService.removeMember：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | security_or_access：一個已被店主明確移除（或曾為 REMOVED）的成員，可能因為並發交易用「舊快照」整列覆寫而復活成 ACTIVE，並帶著該並發交易寫入的角色（例如 STORE_STAFF、甚至 updateMemberRole 賦予的 STORE_MANAGER）繼續留在店鋪。反向（accept 先寫、remove 後寫）則是「使用者以為自己已加入，實際上被靜默打回 REMOVED」，屬於資料不一致但影響較小。核心... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| DEF-144 | TenantService.updateMemberRole：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | security_or_access（存取控制完整性問題，非金額）：當 updateMemberRole 與 removeMember（或兩次併發 updateMemberRole）在同一 TenantMember 列上交錯執行時，較晚 commit 的交易會用「自己那份交易開始時讀到的舊快照」整列覆蓋資料庫，包含它自己沒有主動修改的欄位。具體可觸發後果：(a) Owner 呼叫 removeMember 想踢除某成員，但若此前一瞬間已... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-145 | AdminService.approveTenant：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若真的觸發，後果分兩種且都屬「狀態機/稽核」等級，不涉金額：(a) 兩個 approveTenant 併發：第一個成功寫入 ACTIVE + 建立 6 筆 feature toggle；第二個在 initializeFeatureToggles() 的 check-then-insert（AdminService.java:469-488）會撞上 tenant_feature_toggles(tenant_id, feature_key... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-146 | AdminService.rejectTenant：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若真的被觸發，後果屬於 security_or_access 類：Tenant 最終落在 ACTIVE 還是 REJECTED，會由兩個並發交易的 commit 順序決定而非業務邏輯決定——例如一個被判定該駁回（詐欺／資料不全）的申請店鋪，可能因寫入順序反而變成 ACTIVE 並獲得上架/收款能力。但這是「若觸發才會發生」的假設性後果：目前生產環境沒有任何業務流程會讓一個 Tenant 走到 PENDING_REVIEW 狀態去被審核，... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-147 | AdminService.reviewTenant：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 後果屬存取控制／狀態機類，非金額。reviewTenant 對 Tenant.status 做無條件覆寫（APPROVE→ACTIVE／REJECT→REJECTED），完全不檢查目前狀態，因此若與 updateTenantStatus（ACTIVE↔SUSPENDED/TERMINATED 狀態機）或未來的 approveTenant/rejectTenant 對同一 Tenant 交錯執行，最終狀態由「誰最後 commit」決定，可... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| DEF-164 | TenantService.declineInvite：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若同一被邀請人對同一筆邀請幾乎同時送出 accept 與 decline 兩個請求，且 decline 交易在讀取當下（status 仍為 INVITED）之後、但在 accept 交易 commit 之前完成讀取，兩者最終依提交順序而定：後 commit 的一方會用自己交易開始時讀到的舊快照整列覆寫（TenantMember 無 @Version，save() 是整列 blind UPDATE、非 WHERE status=... 條... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-165 | TenantService.inviteMember：併發競態（security_or_access） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 邊界情況依路徑而異：(1) 全新邀請（DB 尚無該 (tenant_id,user_id) 列）：V1__Initial_Schema.sql 對 tenant_members 有 UNIQUE(tenant_id, user_id) 約束，兩個並發首次邀請即使都通過 `existing.isEmpty()` 檢查各自 INSERT，後提交者會被 DB 唯一約束擋下拋 DataIntegrityViolationException，屬「... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
