...
|----|------|-------------|---------|---------|---------|------|
| DEF-023 | Booking 付款擁有權檢查缺口（IDOR 疑慮） | Sprint 67（撰寫 PaymentStateService 測試時發現） | `PaymentStateService.getBookingPaymentState` 未呼叫任何擁有權檢查（對照 `getOrderPaymentState` 等方法皆有 `checkOrderOwnership`）；同模組 `BookingService.getBooking/updateBooking`、`PaymentService.processBookingPayment` 同樣缺少擁有權檢查。`booking:read` 權限連 GUEST 角色都持有，形同任何登入者可查詢他人 booking 付款狀態。過去 `DEF-018/019`（Sprint 33/36）已修復 Order 側同類 IDOR，當時 commit 訊息明確記載「booking 付款側非本次範圍，如需另立項目評估」，但後續未見對應票證關閉，屬已知但未排入計畫的缺口。**本項目僅記錄調查發現，未修改任何程式碼**——修復涉及是否要在 Booking 讀寫路徑全面加上擁有權檢查，屬於架構/安全範圍決策，超出單一 Sprint 測試補強授權，需 PO/Security owner 決策修復範圍與時程 | 需 PO/Security owner 決策：(a) 是否修復、(b) 修復範圍（僅付款查詢 or 含 BookingService 讀寫全面）、(c) 是否影響現有前端呼叫模式 | 待評估（修復本身估計 3-5，含 BookingService+PaymentService+PaymentStateService 三處） | ⚠️ 待決策 |
