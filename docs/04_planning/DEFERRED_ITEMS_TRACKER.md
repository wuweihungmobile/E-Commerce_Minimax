...
|----|------|-------------|---------|---------|---------|------|
| DEF-038 | `TenantContextFilter` 對 `ADMIN` 角色的 `X-Tenant-ID` header 無驗證信任，可任意切換 `TenantContext` 租戶（架構審查項目） | Sprint 77（探查 `NotificationService` 範圍時發現） | 探查 `NotificationTemplateService` 的租戶隔離邏輯時，追蹤 `TenantContext.getCurrentTenant()` 的信任來源，發現 `TenantContextFilter.resolveEffectiveTenantId()`（`backend/src/main/java/com/nextkey/ecommerce/api/filter/TenantContextFilter.java:84-92`）對 `role` 為 `"SUPER_ADMIN"` 或 `"ADMIN"` 的使用者，只要請求帶有 `X-Tenant-ID` header 就直接信任其值作為當前租戶，完全不驗證該 `ADMIN` 使用者是否真的隸屬/被授權管理該目標租戶。兩輪獨立探查對此是否為「已知且刻意的設計」給出矛盾線索：`RolePermissionMapping.java:108` 註解稱 `ADMIN` 為「租戶內管理」，但過去 `DEF-019/023/024/026/028/029/032/033/036` 等一系列 IDOR 修復，其 `isAdmin()`/`checkXxxOwnership` 判斷邏輯統一把 `ROLE_ADMIN` 與 `ROLE_SUPER_ADMIN` 並列為「可跨租戶放行」的角色——也就是說，**過去至少 9 個既有 DEF 修復所建立的擁有權檢查，其「admin 可跨租戶」放行分支的前提都是 `TenantContext` 裡的租戶值可信，而這個值對 `ADMIN` 角色而言完全由客戶端 header 決定**，未經任何伺服器端授權驗證。若 `ADMIN` 帳號的建立/授權方式無法保證其被綁定到特定租戶（`ADMIN` 目前確實不在 `TenantMember`/租戶成員模型內），此機制形同讓任何持有 `ADMIN` 角色的帳號可偽造 `X-Tenant-ID` 通過上述所有既有擁有權檢查的「本租戶」或「admin」分支，等同繞過整批 IDOR 修復的防線。搜尋全庫（`docs/`、測試檔）未發現任何 DEF、SOP 文件或測試明確審視/驗證過這個機制；`git log` 顯示 `TenantContextFilter.java` 核心邏輯自初始 commit 以來從未被修改或納入後續多輪租戶 IDOR 排查範圍。本 Sprint 依範圍僅記錄現況，不深入調查全站受影響端點清單，也不變更程式碼 | 需業務/架構決策：(1) `ADMIN` 角色的租戶歸屬語意應為「租戶內管理員」還是「平台級跨租戶角色」；(2) 若前者，`TenantContextFilter` 需改為驗證 `ADMIN` 是否真的被授權管理 `X-Tenant-ID` 指定的租戶（例如透過某種 admin-tenant 授權記錄），而非無條件信任 header；(3) 若後者（維持現狀為刻意設計），應在 `RolePermissionMapping.java:108` 修正過時的「租戶內管理」註解並補上明確的架構決策記錄與測試 | 5 | ⚠️ 已記錄，**不排入排程**（待使用者決策，非阻塞 Sprint 77） |
...
| DEF-034 | `CmsService.getPageBySlug`/`getActiveBanners`（公開瀏覽端點）完全不做租戶過濾，跨租戶內容混雜 | Sprint 74（範圍探查時發現） | 探查確認兩個公開方法（`getPageBySlug`、`getActiveBanners`）完全未依 tenantId 過濾，任何租戶已發布的頁面/橫幅會混雜出現在所有訪客的瀏覽結果中；`TenantContextFilter` 對匿名請求恆設為 `SYSTEM_TENANT_ID`（不論是否帶 `X-Tenant-ID` header），因此無法沿用 `TenantContext` 解析「訪客正在瀏覽哪個租戶」。經比對 `PostController`/`PostService.getPublishedPostBySlug`（同屬 CMS 領域，`/v2/posts` 已 `permitAll()`）發現既有慣例是「呼叫端明確傳入 `tenantId` query 參數」，而非依賴 `TenantContext`；另發現 `CmsController` 的 `/v2/cms/**` 並未列在 `SecurityConfig` 的 `permitAll()` 清單中，實際上會落入 `.anyRequest().authenticated()`，與程式碼註解「(公開)」不符，需先確認此為預期行為或遺漏。非跨租戶 IDOR（無寫入/竄改風險，僅公開已發布內容混雜顯示），性質為公開端點租戶範圍設計缺口，涉及是否比照 `PostController` 模式修改 API 簽名（新增必填 `tenantId` 參數）與是否補上 `permitAll()`，需業務/架構判斷，本 Sprint 依範圍僅記錄現況行為測試，不變更程式碼 | 需業務決策：(1) 是否比照 `PostController` 要求呼叫端傳入 `tenantId`；(2) `/v2/cms/pages/{slug}`、`/v2/cms/banners/active`、`/v2/cms/banners/{id}/click` 是否應補上 `SecurityConfig` 的 `permitAll()` | 3 | ⚠️ 已記錄，**不排入排程**（待使用者決策） |
