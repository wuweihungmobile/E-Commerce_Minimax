...
| DEF-044 | 已付款訂單（`PAID`/`REFUNDED`）退款/取消時無庫存自動回補（restock-on-refund） | Sprint 88（AI-2422 訂單庫存串接時，PO 裁示範圍界定為「僅處理預扣→扣帳單向流程」，退款回補列為範圍外） | 本 Sprint 新增的 reserve-at-creation/deduct-at-payment/release-on-cancel-before-payment 三段式庫存機制，僅覆蓋「取消前尚未付款」的釋放情境；已付款訂單走退款流程（`PaymentStateService.refundOrderPayment`）時，先前已透過 `deductStock` 正式扣除的 `totalQty` 目前**不會**因退款而回補，與 M07 結算/退款邏輯有交互，需獨立設計（例如是否應恢復可售庫存、是否與 `SettlementAdjustment` 共用同一觸發點） | 需業務決策：退款是否應自動回補庫存（可售）；若是，回補時點（退款成立時 vs 商品實際retour確認時）需與供應鏈/退貨流程一併考慮 | 2 | ✅ **方向已拍板（2026-09-03）：選項 C——新增「退貨入庫確認」流程，店家實際收到貨並確認可售後才回補庫存**（唯一正確反映實體庫存的做法；順帶會用上 S114 刻意保留的 `RETURN` 異動型別）。待排程 |
