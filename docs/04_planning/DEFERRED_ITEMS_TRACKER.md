...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-046 | 優惠券額度檢查與遞增為非原子操作（讀後寫），高併發下限量券理論上可超發 | Sprint 100（修復總量上限形同虛設時，一併識別出的殘留風險） | Sprint 100 讓 `max_usage_count` 首次真正生效，但 `isUsageLimitReached()` 檢查與 `incrementUsageCount()` 遞增分屬兩次 DB 操作，兩張並行的下單請求可能同時通過檢查。實務影響取決於促銷規模（限量 100 張的券在秒殺情境才有明顯風險）。本專案已有可複用前例：`RateLimitFilter`（Sprint 93 的 Redis Lua 原子操作）與 `IdempotencyService` | 需評估：改用 DB 悲觀鎖/`UPDATE ... WHERE current_usage_count < max_usage_count` 條件更新，或比照 Sprint 93 以 Redis Lua 處理；並確認業務上可接受的超發容忍度 | 3 | ⚠️ 已記錄，待評估 |
