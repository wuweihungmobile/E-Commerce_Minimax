...
|----|------|-------------|---------|---------|---------|------|
| _(無)_ | 高優先級區目前無項目——DEF-060 已於 Sprint 109 修復並結案（見下方「已完成延後項目」） | - | - | - | - | - |
...
| DEF-062 | `SRD_Database_Schema.md` §2.1.1 與 PRD §8.2.1 的 `tenants` 欄位定義與實作的 `V1__Initial_Schema.sql` 全表級不符 | Sprint 110（做 PRD §340 文件同步時，反向掃描帶出） | 設計稿有 8 處欄位級落差：`store_name`／`owner_id`／`business_type`／`approved_at`／`approved_by`／`rejected_at`／`rejected_by` 與兩個 UNIQUE 約束（`uk_tenants_store_name`／`uk_tenants_contact_email`）在實作中**皆不存在**；實作另有 `slug`／`metadata`／`commission_rate` 及 V62（Stripe Connect）／V65（採購審批門檻）追加的欄位。本輪只針對「M17 開店/審核流程」這一個事實加註（`tenants` 沒有申請中狀態、請以 Flyway 為準），**全表對照是另一個題目**，不塞進文件同步輪次。PRD §8.2.1 同樣列有實作不存在的 `owner_id`／`business_type`／`rejection_reason`。⚠️ 這很可能不是 `tenants` 獨有——同一份 SRD／PRD 的其他資料表是否同樣漂移，未查證 | 無 | 3 | 📋 待排程 |
