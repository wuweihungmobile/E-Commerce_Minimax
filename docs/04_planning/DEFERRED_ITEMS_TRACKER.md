...
| DEF-065 | **訂單流程完全不寫庫存流水帳**：PRD §6.7.3 明訂 C 端下單要產生 `StockMovement type=RESERVE`、出貨產生 `OUTBOUND`、取消產生 `RELEASE`，但全專案只有 `PurchaseOrderService`（收貨）與 `StockMovementService`（手動異動）寫入 `stock_movements`；S103 改寫的 `ProductInventoryService` 三段式庫存操作對 `StockMovement` **零引用** | Sprint 114（修復 DEF-063 時查證枚舉使用點所發現） | `product_inventory` 的數字會動，流水帳一筆都不會留 → 庫存台帳（PRD §6.7.3「按 SKU 查看即時庫存、庫存異動記錄」P0）對 C 端所有進出完全沒有記錄，`RESERVE`／`OUTBOUND`／`RELEASE` 三型在 DB 裡從未出現過。**這也解釋了枚舉漂移為何能存活這麼久——那三型從來沒被寫入過，自然沒人發現名字是錯的**。屬補功能而非修 DEF-063 的漂移，依 Rule 3 不順手擴大 | 無需業務決策；需決定寫入時點、`reference_type`／`reference_id` 填法、是否一併補 `before/after_reserved_qty` | 5 | ⚠️ 已記錄，**建議下一輪** |
| DEF-064 | M16 ERP 異動列表的 **SKU／品名／參考單號三欄永遠空白**，且新增表單的「參考單號」輸入送出後被靜默丟棄 | Sprint 114（修復 DEF-063 時比對前後端 DTO 所發現） | `StockMovementDto`（後端）有 `skuCode`／`productName`／`referenceNumber` 三個欄位，但 `StockMovementService.toDto()` **從未填值**，前端列表三欄恆顯示 `-`；另 `StockMovementRequest`（後端）**沒有 `referenceNumber` 欄位**，前端表單送出的該格被 Jackson 靜默忽略。與 DEF-063 同一個畫面但根因不同（DTO 未填值 ＋ 請求欄位缺漏，非枚舉漂移） | 需設計決策：「參考單號」與採購單號／訂單編號的關係為何，是自由文字還是關聯單據 | 2 | ⚠️ 已記錄，**建議下一輪** |
