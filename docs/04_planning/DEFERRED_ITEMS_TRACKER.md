...
**文件版本**: v2.36
**最後更新**: 2026-09-08（**Sprint 145**（v2.36 追加：修 `DEF-167` 查證可見後果時發現並修復 `DEF-168`——功能開關頁面前後端契約 7 欄只有 1 欄對得上，整個頁面實質壞掉；同時更正 `DEF-167` 列中「四個假開關」的不準確描述。E2E 的 `notNullValue()` 寬鬆斷言一併收緊）：兩階段。①開工盤點補登 `DEF-166`/`DEF-167`——這兩筆先前只存在於 Sprint 143/144 的 PLAN 內文，無任何跨 Sprint 追蹤機制，依 [SPRINT_ARTIFACT_CONVENTION.md](../05_development/SPRINT_ARTIFACT_CONVENTION.md) v2.0「Action Items 維持必要」的裁定補登。②實作：`DEF-166` 查證後**更正為非缺陷並結案**（blind write，PRD 無前置條件規則，加 CAS 反而引入缺陷；含「上一輪補登時過度框定」的自我揭露）；`DEF-167` **已修復但查證推翻原描述**——不是「兩份清單需同步」而是「其中一份是死碼、活路徑少建 4 個數值配額」，後果限於前端四個假開關，**無金流損失**。兩項皆移入已完成延後項目。🔴 **同時揭露本文件自身的維護漂移**：本版之前，「文件版本」停在 v2.34（Sprint 137），Sprint 138~144 共七輪完全未更新版本區，與同日發現的 `RELEASE_TRACKER` 斷更 8 個 Sprint 屬同型問題）
