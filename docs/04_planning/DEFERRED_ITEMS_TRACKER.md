...
| DEF-055 | `incrementViewCount` ×3（`PostService` / `FaqService` / `KnowledgeBaseService`）為讀後寫，瀏覽數系統性少計 | Sprint 105（讀後寫模式全專案系統性掃描時發現） | 三處實作完全同型：`findById` → `article.incrementViewCount()` → `save()`，對應實體皆無 `@Version`，故失效模式是**靜默丟失更新**而非拋例外。**這是本次掃描中併發度最高的路徑**（每次文章／貼文瀏覽都會走），因此少計幅度應遠大於 DEF-053 的退款情境。後果不只是統計數字：`KnowledgeArticleRepository:41` 以 `ORDER BY a.viewCount DESC` 取熱門文章，少計會直接讓熱門排名失真。修法與 DEF-053／DEF-054 相同且更單純（單欄位 `viewCount = viewCount + 1` 原子 UPDATE），無需業務決策。本輪未修的理由：屬**排名／分析**而非金額或安全，Sprint 105 已修兩處生產邏輯，依 Rule 3 不順手擴大 | 無前置需求（證據已完整記錄於 [SPRINT_105_PLAN.md](SPRINT_105_PLAN.md) §2） | 2 | ⚠️ 已記錄，建議下一輪處理 |
