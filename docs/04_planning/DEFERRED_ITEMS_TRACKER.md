...
|----|------|-------------|---------|---------|---------|------|
| DEF-093 | `ListingController.getListings()`/`getListing()` 直接序列化裸 `Listing` 實體，`owner`（LAZY → `User`）懶載入會把整個 `User`（**含 `passwordHash`，無 `@JsonIgnore`**）序列化進 API 回應 | Sprint 129（查證 DEF-076 既有 listing 端點時意外發現） | 本輪聚焦 DEF-075~079 五項修復，本項屬過程中查證附帶發現，未排入本輪範圍 | 無（純新增 `@JsonIgnore` 或改回傳 DTO，不需業務決策） | 2 | ⬜ 待排程。全庫未設定 Jackson Hibernate 模組（`Hibernate5Module`/`Hibernate6Module`）、亦未關閉 `spring.jpa.open-in-view`（預設 `true`），故序列化裸實體時懶載入代理會在回應序列化階段被觸發並遞迴序列化整個關聯圖。`Listing.owner`（`User`）首當其衝：`User.passwordHash` 欄位無 `@JsonIgnore`。修法：`ListingController` 改回傳輕量 DTO（比照本輪 DEF-076 新增的 `ListingOptionDto` 做法），或在 `User.passwordHash` 補 `@JsonIgnore`（後者為全域防線，建議兩者皆做）。詳見 [SPRINT_129_PLAN.md](SPRINT_129_PLAN.md) §7 |
...
| DEF-092 | 5 個權限碼僅 SUPER_ADMIN 到得了：`cms:read/create/update/publish`、`notification:create` | Sprint 128（修 DEF-073 時做全庫檢查發現） | 與 DEF-075 性質不同——註解帶 `hasRole('SUPER_ADMIN') or ...` fallback，**不是必定 403** | 產品決策已於 Sprint 129 拍板（比照 `PostController` 開放，見狀態欄），**下一輪可直接實作，無需重新拍板** | 3 | ⬜ 待排程。已由 `PreAuthorizePermissionCoverageTest.superAdminFallbackOnlyCodesAreUnchanged` 自動辨識並守住，新增同類碼會失敗。**Sprint 129 查證並拍板**：`cms:*` 走 `hasRole('SUPER_ADMIN') or hasAuthority(...)` fallback 寫法，與 DEF-073 修復前的 `NOTIFICATION_TEMPLATE_*` 同款，判定為「忘記授權」而非刻意平台專屬；同類模組 `PostController`（`/cms`、`/dashboard/posts`）15 支日常 CRUD 端點已開放 `STORE_OWNER`/`STORE_STAFF`/`SELLER`/`HOST`，僅 3 支管理端點才是裸 `hasRole('SUPER_ADMIN')`——真正刻意平台專屬的對照組是 `AdminController`（22 支端點全部裸 `hasRole('SUPER_ADMIN')`，且 `TENANT_MANAGE`/`ADMIN_PANEL`/`SYSTEM_CONFIG` 三碼全庫無任何 `@PreAuthorize` 引用）。拍板方案：`cms:read/create/update` → STORE_OWNER+STORE_STAFF+SELLER+HOST（比照 PostController CRUD）；`cms:publish` → STORE_OWNER+ADMIN（發布動作限制較高層級）；`notification:create` → STORE_OWNER+ADMIN（該端點前端目前無任何呼叫者，先保守授權）。詳見 [SPRINT_128_PLAN.md](SPRINT_128_PLAN.md) §6、[SPRINT_129_PLAN.md](SPRINT_129_PLAN.md) §1 |
...
**文件版本**: v2.28
**最後更新**: 2026-09-05（Sprint 129：修復 `DEF-075`（dashboard/faq/knowledge/media 13 個權限碼孤兒）與 `DEF-076~079`（ERP 採購單建立/檢視/編輯 + 供應商檢視/編輯四頁契約缺口），五項移入已完成延後項目；`DEF-092` 產品定性已拍板但本輪未實作，維持待排程並補上決策內容；查證過程中新登記 `DEF-093`（`ListingController` 序列化裸實體導致 `User.passwordHash` 潛在洩漏）。詳見 [SPRINT_129_PLAN.md](SPRINT_129_PLAN.md)）
