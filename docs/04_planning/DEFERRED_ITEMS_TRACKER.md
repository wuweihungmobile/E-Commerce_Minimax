...
|----|------|-------------|---------|---------|---------|------|
| _(無)_ | 高優先級區目前無項目——DEF-066 已於 Sprint 116 修復並結案（見下方「已完成延後項目」） | - | - | - | - | - |
...
| DEF-068 | **schema 文件守門只涵蓋「文件裡已經寫到的表」**：`SRD_Database_Schema.md` §2 只有 **14 張表**的 DDL，而 `check_schema_doc.py` 是逐一走訪文件裡的區塊去比對——**文件沒寫的表，漂移完全不會被發現** | Sprint 117（新增 `stock_movements.reference_number` 時發現） | 本輪新增欄位時，守門是靠 **PRD §8.2 那一半**攔下的；`stock_movements` 根本不在 SRD 的 14 張表裡。也就是說「✅ 文件無漂移」的真正意思是「**有寫的那些表**沒漂移」，不是「文件是完整的」。DEF-062（Sprint 111）之所以存在，正是因為 SRD「整整 100 個 Sprint 沒人發現它逐欄都是錯的」——涵蓋率不足會讓同類問題在未被文件化的表上重演 | 需決策：(a) 補齊全部資料表的 SRD DDL（可用 `sync-schema-doc` 產生，但需人工審閱分節與說明）；(b) 讓守門對「實作有、文件沒有」的表**主動報錯**，逼文件跟上；(c) 明確定義 SRD 只涵蓋核心表，並把這個範圍寫進文件 | 3 | ⚠️ 已記錄，**待使用者拍板** |
| DEF-067 | `inventory_checks` 表與 `InventoryCheck` entity 是孤兒：庫存盤點為 PRD §6.7.2 **P1** 功能，實體與資料表都在（`V50` 建），但**沒有任何生產程式碼讀或寫** | Sprint 116（修復 DEF-066 時查證兩張庫存表所發現） | 與 DEF-066 的 `inventory` 表同源——`V50` 檔頭自承兩者都是「entity 存在但沒有建表 migration，導致 ddl-auto=validate 失敗」才補建的。差別在於：`inventory` 是**錯誤的資料來源**（有四個讀取點讀它、讀到空資料），而 `inventory_checks` **沒有任何讀取點**，屬尚未實作功能的骨架，危害等級低得多。刪表會讓 ddl-auto=validate 失敗（entity 仍在），故不隨 V72 一併移除 | 需產品決策：庫存盤點（PRD P1）要實作還是連同骨架一起移除 | 3 | ⚠️ 已記錄，**待使用者拍板** |
| DEF-064 | M16 ERP 異動列表的 **SKU／品名／參考單號三欄永遠空白**，且新增表單的「參考單號」輸入送出後被靜默丟棄 | Sprint 114（修復 DEF-063 時比對前後端 DTO 所發現） | `StockMovementDto`（後端）有 `skuCode`／`productName`／`referenceNumber` 三個欄位，但 `StockMovementService.toDto()` **從未填值**，前端列表三欄恆顯示 `-`；另 `StockMovementRequest`（後端）**沒有 `referenceNumber` 欄位**，前端表單送出的該格被 Jackson 靜默忽略。與 DEF-063 同一個畫面但根因不同（DTO 未填值 ＋ 請求欄位缺漏，非枚舉漂移） | 需設計決策：「參考單號」與採購單號／訂單編號的關係為何，是自由文字還是關聯單據 | 2 | ✅ **方向已拍板（2026-09-03）：選項 C——自動顯示來源單據 ＋ 店家自由文字備註，兩欄都做**。待排程 |
| DEF-044 | 已付款訂單（`PAID`/`REFUNDED`）退款/取消時無庫存自動回補（restock-on-refund） | Sprint 88（AI-2422 訂單庫存串接時，PO 裁示範圍界定為「僅處理預扣→扣帳單向流程」，退款回補列為範圍外） | 本 Sprint 新增的 reserve-at-creation/deduct-at-payment/release-on-cancel-before-payment 三段式庫存機制，僅覆蓋「取消前尚未付款」的釋放情境；已付款訂單走退款流程（`PaymentStateService.refundOrderPayment`）時，先前已透過 `deductStock` 正式扣除的 `totalQty` 目前**不會**因退款而回補，與 M07 結算/退款邏輯有交互，需獨立設計（例如是否應恢復可售庫存、是否與 `SettlementAdjustment` 共用同一觸發點） | 需業務決策：退款是否應自動回補庫存（可售）；若是，回補時點（退款成立時 vs 商品實際retour確認時）需與供應鏈/退貨流程一併考慮 | 2 | 🔄 **Sprint 118 完成後端核心**（狀態機、庫存回補、權限、API、11 個整合測試）；前端排入 Sprint 119。詳見 [SPRINT_118_PLAN.md](SPRINT_118_PLAN.md) |
