...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-094 | `checkout/page.tsx`（純訂房結帳頁）與 DEF-084 相同結構的電話驗證/送出不一致 bug：驗證時 `.replace(/\s/g,'')` 剝空白，送出時只 `.trim()` | Sprint 131（修 DEF-084 時查證發現） | 前端型別/邏輯與 DEF-084 完全相同，但屬不同頁面/檔案，依 Rule 3 精準改動原則另開項目 | 無 | 1 | ⬜ 待排程。`checkout/page.tsx:83-84` 驗證 vs `:135` 送出；後端 `BookingDto.CreateRequest.guestPhone` 同為 `@Pattern(^[0-9]{8,15}$)` |
| DEF-095 | ERP 採購單編輯頁「更新訂單」按鈕是空實作：`onClick={() => {/* update */}}`，點擊完全無反應（含後端本來支援的 notes 也存不了） | Sprint 131（修 DEF-090 時查證發現） | 範圍明顯大於 DEF-090（型別誤導）本身，且需決定 expectedDeliveryDate 在編輯模式要不要一併改唯讀，屬產品/UX 決策 | 需拍板：是否恢復 notes 可編輯、expectedDeliveryDate 顯示方式 | 2 | ⬜ 待排程。`PurchaseOrderForm.tsx:568`，從此元件最早版本就是空實作，非近期回歸 |
| DEF-096 | M18 媒體資產庫（`/v2/media/upload` JSON 端點 + `services/media.ts` 的 `uploadMediaAsset()`）後端與前端 service 皆已完整實作，但從未被任何頁面串接使用 | Sprint 131（修 DEF-089 時查證發現） | 是否要繼續開發串接，還是移除此半成品功能，屬產品決策，非單純接線 bug | 需拍板：串接還是移除 | 3 | ⬜ 待排程。完全獨立於已在生產路徑使用的 `uploadMediaMultipart()`/`/v2/dashboard/media/upload-multipart` |
...
**文件版本**: v2.29
**最後更新**: 2026-09-06（Sprint 131：修復 `DEF-080~091`（12 項契約漂移，含 M09 通知模板建立/編輯 UI、房源搜尋 keyword 失效＋跨租戶範圍缺口修復），十二項移入已完成延後項目；調查過程中新登記 `DEF-094`（checkout 訂房頁電話姊妹缺陷）、`DEF-095`（ERP 採購單編輯按鈕空實作）、`DEF-096`（M18 媒體資產庫半成品功能）。詳見 [SPRINT_131_PLAN.md](SPRINT_131_PLAN.md)）
