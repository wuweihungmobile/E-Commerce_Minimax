...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-116 | AdminService.updateTenantFeatureToggle：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 不是金額/直接安全繞過類，也不是純統計數字，屬於「業務能力開關 + 稽核時間戳」的靜默遺失更新（lost update），且不會自我修復。TenantFeatureToggle（backend/.../domain/model/tenant/TenantFeatureToggle.java）沒有 @Version、也沒有 @DynamicUpdate，因此 Hibernate 對這個 entity 預設用全欄位 UPDATE。當兩個並行... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-118 | ChatService.createConversation：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | Not money/security, but a genuine data-integrity/UX defect: two concurrent "start chat with X" requests from the same initiator can both create a new active Conversation row for the same (initiator, recipient) pair. The ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-119 | CmsService.publishPage：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 非金額/非安全類，屬 CMS 內容狀態遺失（lost update）。ContentPage 無 @Version、無悲觀鎖、無原生條件式 UPDATE，Hibernate 在該實體無 @DynamicUpdate 情況下，flush 時是以整包欄位（含交易一開始讀到的舊值）寫回。情境：管理者 A 呼叫 PUT /v2/cms/pages/{id}（updatePage，改 title/content 等）交易尚未 commit 前，管... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-120 | KnowledgeBaseService.createVersionSnapshot：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 非金額/非安全問題，但也不只是「統計數字」層級：後果是實際的功能性 500 錯誤，會一直持續到有人手動清資料。createVersionSnapshot 直接 save() 一個新建（@GeneratedValue id）的 ArticleVersion 實體，永遠是 INSERT。若兩個併發請求算出相同的 newVersion 並各自成功 INSERT，article_versions 表會出現同一 article_id 底下重複的 ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-121 | KnowledgeBaseService.restoreVersion：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 內容遺失（lost update），非金額/安全類：restoreVersion 會把整個 KnowledgeArticle 實體（title/content/category/isPinned/sortOrder 五欄位是刻意覆寫，但由於實體無 @DynamicUpdate、Hibernate flush 時會用「載入當下」記憶體物件的完整欄位值產生 full-column UPDATE）寫回 DB。若在 restoreVersion... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-122 | KnowledgeBaseService.updateArticle：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 屬「other／內容一致性」而非金額或安全類。KnowledgeArticle 無 @Version 且未用 @DynamicUpdate，updateArticle()（KnowledgeBaseService.java:222-276）對 category/title/slug/content/excerpt/coverImageUrl/status/isPinned/tags 逐一 null-check 後 setXxx，最後整包... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-123 | LogisticsService.createLogistics：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 資料完整性問題（非金額/安全）：同一張訂單可被建立兩筆(以上) Logistics 記錄，違反「一張訂單至多一筆有效物流單」的業務不變量。物流商為 Phase 1 Mock 實作（HCTLogisticsProvider/TCATLogisticsProvider 僅用 UUID.randomUUID() 產生假追蹤號，未串接任何真實外部 API），因此沒有真正的重複出貨成本、沒有金流影響、也不是安全漏洞。但重複的 Logistics ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-124 | OrderService.cancelOrder：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 庫存與優惠券額度會被「重複釋放/退還」，導致庫存台帳失真、產生可被重複銷售的「幻影庫存」，以及優惠券總額度被多退一次（超發風險）。具體機制：(1) 庫存面——ProductInventoryRepository.releaseReservation 是相對遞減的原生 UPDATE（`reserved_qty = GREATEST(reserved_qty - :quantity, 0)`），本身雖然是原子操作、不會 lost-updat... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-125 | PaymentStateService.mockPaymentSuccess：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 庫存數字錯誤（帳實不符），非直接金錢損失也非安全漏洞。同一張訂單被併發觸發兩次「付款成功」時：(1) payments 表沒有唯一約束（order_id, status）或 (order_id) 唯一鍵，兩筆 SUCCESS Payment 都能成功 INSERT；(2) Order 實體沒有 @Version，兩個交易的 UPDATE orders SET status='PAID' 都能成功執行（寫入同一個值，不會互相擋下）；(3)... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-126 | PurchaseOrderService.cancelPurchaseOrder：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | PurchaseOrder.status 欄位可被併發的 receive/cancel 交易靜默覆蓋（lost update），造成「PO 狀態顯示 CANCELLED 但 items.receivedQuantity 已被寫入且庫存已經 increaseTotalQty 增加」或反過來「PO 狀態顯示 RECEIVED/PARTIALLY_RECEIVED 但業務本意是取消」的自相矛盾狀態。實際庫存數量（product_invento... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-127 | PurchaseOrderService.receivePurchaseOrder：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 後果偏向 inventory_or_stock 的資料完整性/內控失效，非直接金流或帳號安全問題。PurchaseOrder 與 PurchaseOrderItem 兩個 entity 皆無 @Version 欄位（已讀原始碼確認），findByIdAndTenantId 是純衍生查詢（無 @Lock、無 SELECT ... FOR UPDATE），receivePurchaseOrder() 最後對 item.setReceived... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-128 | PurchaseOrderService.submitPurchaseOrder：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 兩類後果需分開看：(a) submitPurchaseOrder 對自己的併發（同一張 PO 被兩個提交請求同時打）：兩邊都讀到 DRAFT、都通過 canSubmit()，但目標狀態是由 exceedsApprovalThreshold(tenantId, po.getTotalAmount()) 決定的確定性結果（totalAmount/threshold 在此過程中不變），所以不論哪個交易的 UPDATE 後贏，最終 status... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-129 | PurchaseOrderService.updatePurchaseOrder：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 以 PurchaseOrder.java（無 @Version、無 @DynamicUpdate）為前提，Hibernate 對 managed entity flush 時預設會用『整列所有欄位』組 UPDATE SQL（非只寫有異動的欄位），值全部取自該筆交易記憶體中的實體快照。這使得 updatePurchaseOrder 與 submitPurchaseOrder 的併發不只是『notes 被靜默覆寫』，而是可能造成狀態機被整批... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-130 | ReturnRequestService.approveReturn：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 不是金額／庫存直接受損，而是「靜默遺失更新」造成的操作端資料不一致：approveReturn/rejectReturn/cancelReturnRequest 三者若近乎同時對同一筆退貨單觸發，最終落在 DB 的 status（連同 reviewedBy/reviewedAt/rejectionReason 等同批欄位）由「最後 flush 的那個 transaction」決定，先寫入的那個呼叫端仍會收到 HTTP 200 成功回應（... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-131 | ReturnRequestService.cancelReturnRequest：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 非金流直接損失（本模組設計上明確與 payments 解耦，見 ReturnRequestService 類別註解「與退款流程完全獨立...本服務不讀也不寫 payments」），但會造成 return_requests 這張表的狀態/稽核欄位靜默失真、與庫存流水帳互相矛盾，需人工事後核對。具體機制：ReturnRequest 實體無 @Version（V74 migration 只有 status 的 CHECK IN(...) 約束... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-132 | ReturnRequestService.createReturnRequest：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若同一 OrderItem 被兩個並發的 createReturnRequest 交易命中，且各自申請的數量加總超過該 OrderItem 實際購買量，兩筆交易都會通過「退貨數量不可超過購買量」的檢查並各自成功寫入一筆 ReturnRequest（狀態 REQUESTED）。DB 層完全沒有任何約束能事後擋下這件事：return_requests/return_request_items 的 migration（V74__Create_... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-133 | ReturnRequestService.rejectReturn：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 狀態/稽核不一致，非金流也非庫存損失。rejectReturn（連同 approveReturn、cancelReturnRequest）本身完全不碰庫存或金額——三者只更新 return_requests 的 status/reviewedBy/rejectionReason 等欄位。真實濫用情境：店家 A 按下「核准」與店家 B（或買家）幾乎同時按下「駁回」/「撤回」，兩個交易都在 status 仍是 REQUESTED 的視窗內讀... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-134 | RoomCalendarService.markMaintenance：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | Silent corruption of the shared `Booking` row (status / statusFlags / potentially checkInDate·checkOutDate·totalAmount), not a direct double-charge or overselling. Two sub-cases: (a) the `under_maintenance` display flag ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-135 | RoomCalendarService.releaseDateRange：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | Inventory/data-consistency corruption between the room_calendar table and Booking.statusFlags — not a direct financial or double-booking/double-charge issue. Confirmed concrete failure mode: an admin/host marks a room un... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-136 | RoomCalendarService.unmarkMaintenance：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | Not financial/security. Two possible lost-update outcomes on the same room_calendar row + optionally the paired Booking.statusFlags.under_maintenance advisory flag: (a) an unmarkMaintenance transaction racing with anothe... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-137 | RoomService.clearOpenWindow：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 不是「庫存超賣」而是「Room 整列資料的靜默遺失更新（lost update）」，影響開放窗（訂房可訂區間）以及 updateRoom 同時可能改到的其他 Room 欄位（location/maxGuests/checkInTime/checkOutTime/roomCount/amenities/lat/long）。具體效果：(a) 若 updateRoom 在 clearOpenWindow 之後才 commit，且該次 PUT ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-139 | SupportTicketService.createTicket：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 非金額/非安全性問題，屬「自癒但使用者體驗劣化」類：當兩個併發請求在同一天、同一 tenant-prefix 內幾乎同時呼叫 createTicket 時，generateTicketNumber() 的 countByTicketNumberStartingWith(prefix) 讀取到相同的 count，兩者算出同一組 ticketNumber（如 TK-20260907-001）。SupportTicket.ticketNumb... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-143 | TenantService.updateFeatureToggle：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 功能開關（如 ERP_ENABLED/RETAIL_ENABLED/BOOKING_ENABLED 等）在併發窗口下會發生 lost update：兩個並發請求各自讀到同一個 previousState、各自算出各自的 enabledAt/isEnabled，最後 save() 時後 commit 的一方整列覆蓋前者，資料庫最終值只反映「最後 commit」而非兩次操作的邏輯結果。造成的實際後果是「回應給使用者的 previousSta... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
...
| DEF-148 | AdminService.setFeatureToggle：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 兩種情況需分開看，且都不是「金額／安全」等級的後果：(1) 首次建立 toggle 的併發競態：DB 有 `UNIQUE(tenant_id, feature_key)` 約束（V1__Initial_Schema.sql L85，V6 補建時也保留），所以兩個併發 orElse 建新實體+save 並不會產生「重複列」髒資料，而是後寫入的那筆 INSERT 會撞唯一約束丟出 DataIntegrityViolationExceptio... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-149 | CmsService.createPage：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 自癒型／非資料損毀：createPage 的 check-then-act（findBySlug 未鎖 → build → save）在兩個併發請求送出相同 slug 時，理論上兩者的 findBySlug 都可能在對方 commit 前讀到空結果並繼續往下 build。但 cms_pages 資料表在 V49 migration 已建立 `CONSTRAINT cms_pages_slug_key UNIQUE (slug)`（全域唯... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-150 | CmsService.publishBanner：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 屬「other」類——非金流/安全問題，是 CMS 內容與統計數字的靜默 lost-update。Banner 實體（backend/src/main/java/.../domain/model/cms/Banner.java）沒有 @Version，全專案也沒有任何地方對它加 @DynamicUpdate，因此 Hibernate flush 時預設會用「整列所有欄位」組 UPDATE（值取自 findById 當下載入 Java 物... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-151 | CmsService.updateBanner：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | CMS 橫幅內容遺失更新（lost update），非金額／非安全性問題。Banner 實體（Banner.java）沒有 @Version，updateBanner()（CmsService.java:247-258）走「findById 讀取整包實體 → 依 request 各欄位是否為 null 逐一 setXxx（拆成 updateBannerTextFields/LinkFields/ScheduleFields/Displa... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-152 | CmsService.updatePage：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | CMS 內容資料的靜默 lost update（非金額/非安全類）。ContentPage 實體無 @Version、也未加 @DynamicUpdate，Hibernate 預設在 flush 時對「所有」mapped 欄位下 UPDATE（而非只更新有 dirty 的欄位）。updatePage 的流程是 findById 讀取整包實體到記憶體 → 依 request 各欄位 null 判斷只 set 部分欄位 → save()。若... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-153 | KnowledgeBaseService.createArticle：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 不是資料損毀，也不是金額/安全問題。knowledge_articles 有 UNIQUE(tenant_id, slug) 的 DB 約束（V24 migration 第 24 行已確認），所以併發下絕不可能真的產生兩篇同 slug 的文章——這點初步描述是對的。但真正的後果被低估了：輸掉的那個併發請求會因 INSERT 違反唯一約束而拋出未被攔截的 DataIntegrityViolationException（RuntimeExc... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-154 | KnowledgeBaseService.createCategory：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 沒有資料毀損或重複資料：V29 migration 在 knowledge_categories 上建立了 UNIQUE (tenant_id, slug) 約束（取代舊的全域 slug unique），所以競態下最壞情況並非真的產生重複分類。實際後果是「敗方」請求會收到未被特別處理的 DataIntegrityViolationException：GlobalExceptionHandler 只針對 BusinessException... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-155 | KnowledgeBaseService.deleteCategory：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 在極窄的併發窗口下（deleteCategory 第 141 行的 SELECT 檢查與第 146 行的實際 DELETE 之間，另一請求剛好對同分類 createArticle 並先行 commit），deleteCategory 的 categoryRepository.delete(category) 會因 knowledge_articles 對 knowledge_categories 的 FK RESTRICT（V24 mi... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-156 | KnowledgeBaseService.schedulePublish：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | Lost-update on non-critical CMS content, not money/security. KnowledgeArticle has no @Version field (confirmed by grep — zero matches), no pessimistic lock, no native atomic UPDATE for this path. schedulePublish (line 38... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-157 | KnowledgeBaseService.updateCategory：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | Lost-update on knowledge-category metadata only (name/slug/description/icon/sortOrder) — no money, no auth/security impact. If two PUT requests to the same category overlap (e.g. one admin renames the category while anot... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-158 | LogisticsService.cancelLogistics：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 確認為真實的 lost-update 競態，但影響範圍侷限於 Logistics 資料列本身的顯示欄位，不會外溢到金流或安全性。具體機制：Logistics entity（Logistics.java）沒有 @Version，V51__Create_Logistics_Table.sql 也只有 enum CHECK constraint、無狀態機約束或版本欄位；Hibernate 未加 @DynamicUpdate，故 save() ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-159 | LogisticsService.updateLogisticsStatus：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若真的觸發競態，後果僅止於「顯示層/追蹤資訊不一致」的自癒型問題，並非金額或安全問題：Logistics.status 可能在被推進到 DELIVERED 後又被較早事件的後寫覆蓋回 IN_TRANSIT/PICKED_UP 等非終態，使買賣雙方看到的物流追蹤頁（trackLogistics/getTrackingDetail）短暫顯示錯誤狀態。但 Order.status=DELIVERED 這個真正驅動業務邏輯（結算 Settlem... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-160 | MediaService.createCategory：併發競態（stats_or_cosmetic） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 純粹是統計/外觀層級的資料完整性瑕疵，非金額也非安全問題。兩個並發請求可各自建立一筆同名（同 tenant、同 parent，或同 tenant 皆為根分類）的 media_categories 記錄，結果是後台分類清單/下拉選單出現兩筆顯示名稱相同但 id（UUID）不同的分類。由於所有後續操作（掛媒體檔案、父子關聯、更新、刪除）都以 UUID id 而非 name 定位，重複名稱不會造成資料遺失、金流錯誤或權限繞過，僅造成使用者在 ... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-161 | OrderService.createRoomOrder：併發競態（inventory_or_stock） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 若此路徑被觸發，效果等同無庫存保護的超訂：createRoomOrder（OrderService.java:309-357）只用 request 帶入的 checkInDate/checkOutDate 與 Listing.basePrice 算 pricePerNight×nights，直接 build+save 一筆 Order（金額計算、無任何 RoomCalendarService 呼叫、無日期鎖定、無可用性檢查、無日曆佔用寫... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-162 | PaymentStateService.markStripeRefunded：併發競態（financial） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 無金流/安全後果。markStripeRefunded（PaymentStateService.java:294-325）只做「記錄」：設定 payment.status=REFUNDED、選擇性覆寫 stripeRefundId，並在 Order 可轉態時寫 order_state_log。它完全不碰 payment.refundedAmount，也不呼叫 Stripe API——真正的金流動作（Stripe Refund.creat... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
| DEF-163 | SupportTicketService.updateStatus：併發競態（other） | Sprint 136（併發競態全掃，Workflow 對抗性驗證發現） | 非金流/非安全（屬「other」）：SupportTicket 無 @Version、SupportTicketRepository 只是 plain JpaRepository（無悲觀鎖、無原生條件式 UPDATE），save() 對整個實體做欄位覆寫式 UPDATE，DB 未覆寫隔離層級（Postgres 預設 READ_COMMITTED），無任何原子保護。真實可造成的後果：(1) resolvedAt 遺失——客服A將 IN_P... **[僅通過 Workflow 自動化分析，未經主控 session 逐筆獨立複核，見 SPRINT_136_PLAN.md §1/§8]** | 無（重新獨立複核後可比照本輪已修復的 19 筆手法直接修復） | 1 | ⚠️ 已記錄，**不排入排程**（Sprint 136 併發競態全掃發現，規模超出當輪可安全消化的量） |
