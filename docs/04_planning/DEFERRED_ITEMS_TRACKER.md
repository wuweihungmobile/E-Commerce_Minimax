...
|----|------|-------------|---------|---------|---------|------|
| DEF-060 | **Admin 開店審核台前端接在失效流程上**：網友送出的開店申請，Admin 在 UI 上永遠看不到、也無法核准 | Sprint 108（修 DEF-059 追查 `PENDING_REVIEW` 可達性時發現） | 網友申請 → 寫入 `tenant_applications`（PENDING）。但 admin 前端 `/admin/tenants`「審核中」分頁查的是 `GET /v2/admin/tenants?status=PENDING_REVIEW`、`/admin/tenants/[id]/review` 的核准鈕打的是 `POST /v2/admin/tenants/{id}/approve`——**全部接在 Sprint 97 已知永遠等不到資料的舊 `Tenant` 流程上**。真正能核准的 `/v2/admin/tenant-applications/*`（S97 新增）**全前端零引用**（`grep -r` 於 `frontend/` 無任何命中）。S97 補上了後端橋接並記錄「舊端點永遠等不到資料」，但沒有改前端，缺口因此未被關閉。M17「開店申請 + 審核」PRD 標為 P0 | 需產品拍板修法：(a) 改前端審核台接 `tenant-applications` 端點（推薦，順著 S97 既有設計）；或 (b) 依 PRD §340 生命週期圖改回申請時即建立 `PENDING_REVIEW` 租戶（推翻 S97 設計、`tenant_applications` 變冗餘）。屬功能開發非測試強化，未在本輪自行決定 | 5 | ⚠️ 已記錄，待產品決策 |
...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-058 | `at-m17-002`（Admin 審核開店申請）E2E 的前提——列表中存在 `PENDING_REVIEW` 狀態的租戶——**是一次性固件，被測試自己消耗掉** | Sprint 107（E2E 守門擋下 release、追根因時發現） | 🔴 **Sprint 108 更正根因**：S107 原記錄「測試自己建立不出來、資料只能來自 `at-m17-001` 副作用或環境殘留」**經查證不成立**。真正的來源是 Flyway `V7__M17_Test_Data_Init.sql` 明確為本測試播下的**固定一筆** `slug='pending-test'` / `status='PENDING_REVIEW'` 租戶。版本化遷移只跑一次，測試核准它之後該筆就變成 `ACTIVE` 且不會復原——**單次性固件被消耗**，這才是 skip／pass／fail 三態擺盪的成因（乾淨 DB 首跑 pass，之後永遠 skip）。S107 已將該 spec 收斂為「前提成立就真的測、不成立就乾淨 `test.skip()`」，不阻擋 release | ⚠️ **已被 DEF-060 阻擋**：此 spec 驅動的 admin 審核台本身接在 Sprint 97 已知失效的 `Tenant.PENDING_REVIEW` 流程上。若 DEF-060 決定改接 `tenant-applications`，本 spec 需整支改寫；先修測試會被丟棄，故等 DEF-060 拍板後一併處理 | 2 | ⚠️ 已記錄，待 DEF-060 |
