...
| DEF-063 | M16 ERP 手動庫存異動的**前後端異動類型枚舉是兩組不同的值**：前端 7 個選項有 5 個後端不認識（含預設選中的那個），送出必定 E_7005；另 `RETURN` 型別為靜默 no-op | Sprint 113（修復 DEF-051 時核對 PRD §6.7.4 異動類型表所發現） | 前端 `services/erp/stockMovement.ts` 的 `StockMovementType` 用 **PRD §6.7.4 的名稱**（`INBOUND`／`OUTBOUND`／`ADJUST_PLUS`／`ADJUST_MINUS`／`SCRAP`／`TRANSFER_IN`／`TRANSFER_OUT`），後端 `StockMovement.MovementType` 用另一組（`PURCHASE_RECEIPT`／`SALE`／`RESERVATION`／`RELEASE`／`ADJUSTMENT`／`DAMAGE`／`TRANSFER_IN`／`TRANSFER_OUT`／`THEFT`／`RETURN`），中間**沒有任何轉換層**——`StockMovementRequest.movementType` 是 `String`，直接進 `MovementType.valueOf()`。因此 `/dashboard/erp/stock-movements/new` 下拉選單的 7 個選項只有 `TRANSFER_IN`／`TRANSFER_OUT` 可用，其餘 5 個（**含表單預設值 `INBOUND`**）必定拿到「Invalid movement type」。另 `RETURN` 後端可解析但 `switch` 落到 `default`，庫存不動卻照樣寫一筆 `isInbound()` 為真的異動記錄。**使用者可見、必然發生**，但**要往哪邊對齊是產品決策**，故不在 S113 順手改（Rule 3） | 需業務決策：(a) 改後端枚舉對齊 PRD 用語 → 需 migration 轉換 `stock_movements.movement_type` 既有資料；(b) 改前端送後端現有名稱 → UI 標籤與 PRD 用語脫節；(c) 加轉換層。另 `RETURN` 的庫存語意 PRD §6.7.4 未定義，需一併拍板 | 3 | ⚠️ 已記錄，**待使用者拍板** |
