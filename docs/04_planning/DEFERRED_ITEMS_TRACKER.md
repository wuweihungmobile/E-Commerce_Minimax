...
|----|------|-------------|---------|---------|---------|------|
| _(無)_ | 高優先級區目前無項目——DEF-050 已於 Sprint 103 修復並結案（見下方註記） | - | - | - | - | - |
...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-051 | M16 ERP 的 `StockMovementService.createManualMovement` / `PurchaseOrderService.createInboundMovement` 對 `product_inventory` 仍是「載入 → `addStock()`/`deductStock()` → `save()`」讀後寫 | Sprint 103（修復 DEF-050 時確認 ERP 側走另一條路徑） | 本輪範圍界定為訂單流程的三段式庫存（DEF-050），依 Rule 3 不順手擴大到 ERP 後台。ERP 兩處與訂單流程共用同一張 `product_inventory` 表，但仍走 JPA `save()`，只靠 `@Version` 保護，併發下同樣會拋 `ObjectOptimisticLockingFailureException`。**沒有資料正確性風險**——Sprint 103 的三條原生 UPDATE 都帶 `version = version + 1`，ERP 端不會拿過期快照覆蓋訂單流程的寫入——只有「後台併發操作偶發失敗」的可用性問題，且 ERP 後台的併發程度遠低於買家結帳 | 無前置需求——修法與 Sprint 103 完全相同（條件式／相對 UPDATE），但需要 ERP 情境自己的紅燈驗證 （進貨入庫與手動異動的併發語意、失敗處理與訂單流程不同） | 2 | ⚠️ 已記錄，**不排入排程** |
