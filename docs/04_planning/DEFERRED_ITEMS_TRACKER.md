...
|----|------|-------------|---------|---------|---------|------|
| DEF-188 | 賣家訂單管理（列表+詳情+出貨）全站無前端入口，且後端本身也缺租戶範圍查詢端點與讀取授權（`OrderService.getOrder` 未比照 `updateOrderStatus` 補 same-tenant 分支） | Sprint 150（自選掃描發現，追加調查確認規模） | `GET /v2/orders` 是買家專用（`getUserOrders` 固定查呼叫者自己的訂單），沒有任何端點能列出「當前租戶收到的訂單」；`frontend/src/app/dashboard/` 完全沒有 orders 子目錄；`POST /v2/logistics`（建立物流單/出貨）功能完整但因無入口永遠無法被觸發。使用者已拍板排入 Sprint 151，範圍見 [SPRINT_150_PLAN.md](SPRINT_150_PLAN.md) §4 | 無（`orderRepository.findByTenantIdOrderByCreatedAtDesc` 已存在，可重用） | 5 | ⚠️ 已記錄，**已拍板排入 Sprint 151** |
