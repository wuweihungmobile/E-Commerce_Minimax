...
| ID | 標題 | 原始 Sprint | 延後原因 | 前置需求 | 預估 SP | 狀態 |
|----|------|-------------|---------|---------|---------|------|
| DEF-058 | `at-m17-002`（Admin 審核開店申請）E2E 的前提——列表中存在 `PENDING_REVIEW` 狀態的租戶——**測試自己建立不出來** | Sprint 107（E2E 守門擋下 release、追根因時發現） | Sprint 107 已修掉該 spec 的三個真缺陷（`:has-text` 子字串比對誤中列表頁的「已核准 (N)」tab、`waitForLoadState('domcontentloaded')` 在 client-side 導航下不保證任何事、`expect(x || true)` 恆真斷言），但**強迫它跑起來之後才發現前提造不出來**：`POST /v2/tenants/apply`（Guest 端點）建立的是 `TenantApplication`（status=`PENDING`），而 admin 列表與核准鈕看的是 `Tenant`（status=`PENDING_REVIEW`）——**兩個不同的實體**。該 spec 的資料因此只能來自並行執行的 `at-m17-001` 副作用或環境殘留，這正是它長期在 **skip／pass／fail 三態擺盪**的原因（S105 記 54/6、S106 記 53/7、S107 首跑 1 failed）。本輪已收斂為「前提成立就真的測（保留精確 locator 與真斷言）、不成立就乾淨 `test.skip()`」，**不再假失敗、不阻擋 release**，故不急 | 需找出建立 `PENDING_REVIEW` 租戶的實際路徑（前端 `/tenant/apply` 走的可能是另一條），或為 E2E 提供專用 seeding 端點／固件 | 2 | ⚠️ 已記錄，待處理 |
