...
| DEF-183 | `BusinessException.getUserMessage()` 固定回傳 `ErrorCode.getMessage()` 原始模板，帶 `%s`/`%d` 佔位符的錯誤訊息從未被實際代入值就回給前端 | Sprint 147（實作 `E_2009` 配額錯誤碼時，查證是否該讓訊息帶動態數字，追蹤 `BusinessException`/`GlobalExceptionHandler` 全鏈路後發現） | 全庫僅 `E_1088`/`E_1089`/`E_1090`（皆 Review 模組）的訊息模板帶 `%d`/`%s`。`ReviewService` 呼叫端雖有用 `String.format(ErrorCode.E_1088.getMessage(), ...)` 组出格式化字串，但把結果傳進 `new BusinessException(errorCode, details)` 的 `details` 參數；`GlobalExceptionHandler.handleBusinessException` 回給前端的是 `ex.getUserMessage()`，其實作固定 `return errorCode.getMessage();`（原始未代入模板），完全不讀 `details`。故使用者收到的實際文字是字面上的「評價圖片數量超過上限（最多 9 張，目前 %d 張）」，`%d` 從未被替換。另確認 `ErrorCode.getFormattedMessage()` 方法本身全庫零呼叫點，是死碼。本 Sprint 新增的 `E_2009` 因此刻意設計為不帶佔位符的純靜態訊息，避免重蹈覆轍，但既有 3 筆訊息本身未修復 | 需先決定修法方向（(a) `BusinessException` 改讓 `getUserMessage()` 優先回傳 `details`；(b) 呼叫端不再自行 `String.format`，改直接把動態值傳給尚是死碼的 `getFormattedMessage()` 並讓它成為真正的入口；兩者皆會改變既有 API 回應內容，需評估是否有前端/測試依賴目前的原始模板字串） | 2 | ⚠️ 已記錄，**不排入排程**（既有缺陷非本輪引入，範圍局限在 Review 模組 3 筆訊息） |
| DEF-184 | `DashboardListingController.createListing` 的 `RETAIL_ENABLED`/`BOOKING_ENABLED` Feature Toggle 檢查放在 Controller 層，與 PRD §4.4「具體實作約束」明文規定的「不得在 Controller 層執行」相牴觸 | Sprint 147（實作 `MAX_PRODUCTS`/`MAX_ROOMS` 配額檢查、決定該放哪一層時，比對既有布林開關檢查的落點發現） | `ProductController.createProduct`/`RoomController.createRoom` 兩個獨立端點的 `checkFeatureEnabled` 呼叫都正確放在 Service 層（`ProductService.createProduct`/`RoomService.createRoom`），但 `DashboardListingController.createListing`（`POST /v2/dashboard/listings`，前端 `lib/api.ts` 的 `API_ENDPOINTS.create` 實際指向的正式建立商品/房源路徑）卻是在 Controller 方法內直接呼叫 `featureToggleService.checkFeatureEnabled(...)` 後才轉呼叫 `createProductFromDashboard`/`createRoomFromDashboard`——這兩個被轉呼叫的 Service 方法本身完全不含任何 feature toggle 檢查。功能行為正確（切了 toggle 確實會擋下），純粹是分層規範的落差，非缺陷。本 Sprint 新增的 `MAX_PRODUCTS`/`MAX_ROOMS` 配額檢查刻意改放進 `createProductFromDashboard`/`createRoomFromDashboard` 這兩個 Service 方法內（遵循 PRD 規範），未動既有 Controller 層的布林開關檢查（避免範圍蔓延） | 無（把 `featureToggleService.checkFeatureEnabled` 呼叫從 Controller 搬進對應的 `createProductFromDashboard`/`createRoomFromDashboard` 內即可，屬單純搬移不改變行為） | 1 | ⚠️ 已記錄，**不排入排程**（純架構規範落差，行為正確，非本輪配額功能範圍） |
---
