...
雲端的預期效益**尚未實測**，本輪不寫成既定事實（這正是 S103 的教訓）。可推論的部分：整合 job 少掉重複的單元測試（實測 4m25s）是確定的；fork 重用在雲端的倍率則未必與本機相同（雲端 runner 較慢、記憶體與 CPU 配置不同）。**待下一輪開工時以實際 run 回填**。
...
| # | 項目 | 說明 |
|---|------|------|
| 1 | **回填雲端 CI 實測**（下一輪開工） | 本輪只有本機數字。需確認兩件事：(a) 整合 job 是否真的少掉 4m25s 的重複單元測試；(b) fork 重用在雲端 runner 的倍率。**在回填前，不得把本機的 −78% 當成雲端效益引用** |
