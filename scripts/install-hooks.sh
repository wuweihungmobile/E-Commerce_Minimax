...
echo "✅ 所有 Hooks 安裝完成！"
echo ""
echo "📋 Hook 功能說明（v6，2026-07-04）："
echo "  📝 pre-commit: 快速檢查（lint + compile + 核心測試 + secret 掃描，約 1-2 分鐘）"
echo "  📋 commit-msg: 基本訊息檢查（防止空訊息）"
echo "  📤 pre-push:   ⭐ 輕量守門 = make validate-push（backend-unit + schema 漂移 + frontend，~8-12 分）"
echo ""
echo "🔐 安全機制："
echo "  ✅ commit 快速、迭代順暢（pre-commit 快檢；push 跑輕量守門）"
echo "  ✅ 上 GIT = 輕量守門：backend-unit（完整單元測試）+ schema 漂移 + frontend，沒過就擋下"
echo "  ✅ 完整 backend-integration（41 支整合測試）+ E2E 交給雲端 push 觸發執行（act-compat.yml）"
echo "  ✅ 30 分鐘內 + tree-hash 相符的 make validate-push 記錄可直接放行（不重跑）"
echo "  ✅ 純文件/設定（無 backend/frontend code）→ 略過輕量守門"
