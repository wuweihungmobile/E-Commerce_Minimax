...
echo "✅ 所有 Hooks 安裝完成！"
echo ""
echo "📋 Hook 功能說明（v5，2026-06-30）："
echo "  📝 pre-commit: 快速檢查（lint + compile + 核心測試 + secret 掃描，約 1-2 分鐘）"
echo "  📋 commit-msg: 基本訊息檢查（防止空訊息）"
echo "  📤 pre-push:   ⭐ 唯一嚴格門 = 完整測試程序 make validate-release（act + schema + e2e，~30-45 分）"
echo ""
echo "🔐 安全機制："
echo "  ✅ commit 快速、迭代順暢（pre-commit 快檢；push 才跑完整測試程序）"
echo "  ✅ 上 GIT = 完整測試程序：act backend+frontend + schema 漂移 + e2e，沒過就擋下"
echo "  ✅ commit 勤、push 少（批次）：完整成本只在檢查點付一次"
echo "  ✅ 30 分鐘內 + tree-hash 相符的 make validate-release（FULL）記錄可直接放行（不重跑）"
echo "  ✅ 純文件/設定（無 backend/frontend code）→ 略過完整測試"
